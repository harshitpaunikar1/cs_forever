Title: Deferred work — The Linux Kernel  documentation
Mapped Topic: Kernel labs and lectures
Source URL: https://linux-kernel-labs.github.io/refs/heads/master/labs/deferred_work.html
Source Type: official_course_material
Trust Score: 95
Fetched At: 2026-04-17T07:06:08+00:00
Mapped From CSE.md Section: Part 2: C. Operating systems, Linux internals, kernel understanding

# Content

# Deferred work[¶](https://linux-kernel-labs.github.io#deferred-work)
...
**Initialization**. Each type is described by a structure whose fields will have to be initialized. The handler to be scheduled is also set at this time.**Scheduling**. Schedules the execution of the handler as soon as possible (or after expiry of a timeout).**Masking**or**Canceling**. Disables the execution of the handler. This action can be either synchronous (which guarantees that the handler will not run after the completion of canceling) or asynchronous.
...
## Softirqs[¶](https://linux-kernel-labs.github.io#softirqs)
...
### Tasklets[¶](https://linux-kernel-labs.github.io#tasklets)
...
by device drivers. A tasklet is represented by ```
struct
tasklet
```
...
### Timers[¶](https://linux-kernel-labs.github.io#timers)
...
## Workqueues[¶](https://linux-kernel-labs.github.io#workqueues)
...
- it schedules a task to run at a later time`struct delayed_work`
...
## Kernel threads[¶](https://linux-kernel-labs.github.io#kernel-threads)
...
### 1.Timer[¶](https://linux-kernel-labs.github.io#timer)
...
For more information review the [Timers](https://linux-kernel-labs.github.io#timers) section.

### 2. Periodic timer[¶](https://linux-kernel-labs.github.io#periodic-timer)
...
### 3. Timer control using ioctl[¶](https://linux-kernel-labs.github.io#timer-control-using-ioctl)
...
Review [ioctl](https://linux-kernel-labs.github.io/so2/lab3-device-drivers.html#ioctl) for a way to access the ioctl argument.

Note

Review the [Timers](https://linux-kernel-labs.github.io#timers) section for information on enabling /
disabling a timer. In the timer handler, display the current
process identifier (PID) and the process executable image name.

Hint

You can find the current process identifier using the *pid*
and *comm* fields of the current process. For details,
review proc-info.

Hint

To use the device driver from userspace you must create the
...
### 4. Blocking operations[¶](https://linux-kernel-labs.github.io#blocking-operations)
...
### 5. Workqueues[¶](https://linux-kernel-labs.github.io#workqueues-1)
...
. Follow the sections
marked with **TODO 3** in the skeleton and review the [Workqueues](https://linux-kernel-labs.github.io#workqueues)
section if needed.
...
### 6. Kernel thread[¶](https://linux-kernel-labs.github.io#kernel-thread)
...
to create and run the thread`kthread_create()`

to create a suspended thread and then start it running with`wake_up_process()`

.

Review the [Kernel Threads](https://linux-kernel-labs.github.io#kernel-threads) section if needed.
...
Use atomic variables for flags. Review [Atomic variables](https://linux-kernel-labs.github.io/so2/lab2-kernel-api.html#atomic-variables).
