Title: Recommended Setup — The Linux Kernel  documentation
Mapped Topic: Kernel labs and lectures
Source URL: https://linux-kernel-labs.github.io/refs/heads/master/info/vm.html
Source Type: official_course_material
Trust Score: 95
Fetched At: 2026-04-17T07:06:00+00:00
Mapped From CSE.md Section: Part 2: C. Operating systems, Linux internals, kernel understanding

# Content

# Recommended Setup[¶](https://linux-kernel-labs.github.io#recommended-setup)
...
# Virtual Machine Setup[¶](https://linux-kernel-labs.github.io#virtual-machine-setup)
...
package is not strictly required, but will make the virtual machine
faster by using KVM support (with the `-enable-kvm`

option to QEMU). If `kvm`
...
By default, `core-image-minimal-qemu`

it used. This setting can be changed by
updating the `YOCTO_IMAGE`

variable in `tools/labs/qemu/Makefile`

.

## Starting the Virtual Machine[¶](https://linux-kernel-labs.github.io#starting-the-virtual-machine)

You start the virtual machine in the `tools/labs/`
...
command will compile the kernel image and it
...
The virtual machine setup scripts and configuration files are located
in `tools/labs/qemu/`

.

## Connecting to the Virtual Machine[¶](https://linux-kernel-labs.github.io#connecting-to-the-virtual-machine)

Once the virtual machine is started you can connect to it on the serial port. A
symbolic link named `serial.pts`

is created to the emulated serial port
device:
...
On the host you use the `minicom`

command to connect to the virtual machine
via the `serial.pts`
...
When you connect to the virtual machine, simply enter `root`
