...
Source URL: https://pages.cs.wisc.edu/~remzi/OSTEP/file-raid.pdf
Source Type: official_open_book
Trust Score: 97
Fetched At: 2026-04-17T06:55:38+00:00
...
CRUX : H OW TO MAKE A L ARGE , F AST, R ELIABLE DISK
...
ASIDE : T HE RAID M APPING PROBLEM
...
Speciﬁcally , let’s calculate S and R given the following disk characteris-
...
210 ms = 47.62 M B/s
As we can see, because of the large time spent transferring dat a, S is
...
and disk 3 do as well; the data is striped across these mirror pai rs. In
fact, you may have noticed that there are a number of different wa ys to
place block copies across the disks. The arrangement above is a com mon
one and is sometimes called RAID-10 (or RAID 1+0 , stripe of mirrors )
because it uses mirrored pairs (RAID-1) and then stripes (RAI D-0) on top
...
pacity . WithN disks of B blocks, RAID-1 useful capacity is (N ·B)/2.
...
ASIDE : T HE RAID C ONSISTENT -U PDATE PROBLEM
...
Pnew will be set to 0; if (P old == 0), P new will be set to 1. We can express
...
8 9 10 11 P2
12 ∗13 14 15 +P3
Figure 38.6: Example: Writes T o 4, 13, And Respective Parity Blocks
...
where it is better, it has almost completely replaced RAID-4 inthe market-
...
achieve full bandwidth. Think about this to see if you can ﬁgure out why .
© 2008–23, A RPACI -D USSEAU THREE
EASY
PIECES
...
ducing a landmark product in storage, the write-anywhere ﬁle layout or WAFLﬁle system that underlies
the NetApp ﬁle server.
[K86] “Synchronized Disk Interleaving” by M.Y . Kim. IEEE Transactions o n Computers, V ol-
...
ACM Transactions on Computer Systems, 2(2):145–154, May 1984. Finally, a paper that is not
