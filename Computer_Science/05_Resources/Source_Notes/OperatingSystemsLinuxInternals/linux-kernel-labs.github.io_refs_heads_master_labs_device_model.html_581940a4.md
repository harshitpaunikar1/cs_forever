...
Source URL: https://linux-kernel-labs.github.io/refs/heads/master/labs/device_model.html
Source Type: official_course_material
Trust Score: 95
Fetched At: 2026-04-17T07:06:10+00:00
Mapped From CSE.md Section: Part 2: C. Operating systems, Linux internals, kernel understanding

# Content

# Linux Device Model[¶](https://linux-kernel-labs.github.io#linux-device-model)
...
## sysfs[¶](https://linux-kernel-labs.github.io#sysfs)
...
## Basic Structures in Linux Devices[¶](https://linux-kernel-labs.github.io#basic-structures-in-linux-devices)
...
### The kobject structure[¶](https://linux-kernel-labs.github.io#the-kobject-structure)
...
Note that this structure includes a `kobject`
...
structure, which will appear in sysfs, using the
`kobject_set_name()`
...
, or decrementing if it is no longer used using
`kobject_put()`
...
(for example, cdev).
The method is called `release`

and is associated with the object via the ktype
...
and `struct device_driver`

.

### Buses[¶](https://linux-kernel-labs.github.io#buses)
...
function (formerly `hotplug`

) is used with hotplug devices.

Bus operations are the registration, the implementation of the operations
described in the `struct bus_type`

structure and the iteration and
inspection of the devices connected to the bus.

A bus is registered using `bus_register()`
...
function is used when a new device or a new driver is added to the
bus. Its role is to make a comparison between the device ID and the driver ID.
The `uevent`

function is called before generating a hotplug in user-space and
...
being stored in the private data of the driver, the `subsys_private *p`

field),
these can be iterated using the `bus_for_each_dev`
...
Typically, an attribute is defined by the `BUS_ATTR`
...
An example of defining an attribute for `my_bus`
...
The bus is represented by both a `bus_type`
...
### Devices[¶](https://linux-kernel-labs.github.io#devices)

Any device in the system has a `struct device`
...
, the bus it is connected to, the device driver, and a
...
,
the macro `DEVICE_ATTR`

for definition, and the functions
`device_create_file()`
...
One important thing to note is that the `struct device`
...
As seen, the functions `my_register_device`

and `my_unregister_device`
...
from driver) and the function `my_register_device`

will be called to add a
device to the bus.

To use the bus defined above in the driver implementation, we must define a
structure of type `my_device`

, initialize it and register it using the function
...
### Drivers[¶](https://linux-kernel-labs.github.io#drivers)
...
you can find the driver associated with
each device, in `/sys/class`

the drivers belonging to a class, in
`/sys/bus`
...
As before, we have the functions `driver_register()`

and
`driver_unregister()`

to register/unregister a driver.

To work with attributes, we have the `struct driver_attribute`

structure,
the macro `DRIVER_ATTR`

for definition, and the functions
`driver_create_file()`

and `driver_remove_file()`

functions for
adding the attribute to the device.

As with devices, the structure `struct device_driver`
...
and `my_unregister_driver`

to associate with the bus.

To use the functions (in the driver implementation), we must declare a structure
of type `my_driver`

, initialize it and register using the function exported
by the bus.
...
### Classes[¶](https://linux-kernel-labs.github.io#classes)
...
describes a class associated with a device.
...
The advantage of using classes is that the `udev`

program in userspace, which we
...
structure,
and assigns the generic `class`

structure and the device received as a
parameter to it;
In addition, it will create an attribute of the class, `dev`
...
file to create a node in the `/dev`

directory by calling `makenod`
...
a node will be created in the `/dev`

directory.
For the example above, the node `/dev/myclass0`

will be generated.

### Hotplug[¶](https://linux-kernel-labs.github.io#hotplug)

`Hotplug`

describes the mechanism for adding or removing a device from the
...
to the event will be called (usually, the `uevent`
...
utility in the `/etc/udev/`

directory. Different rules can be specified to
capture only certain events and perform certain actions, depending on the
system variables set in the kernel or in `uevent`

functions.

An important consequence is that in this way the plug and play mechanism can be
achieved; with the help of `udev`

and the classes (described above), entries
in the `/dev/`

directories can be automatically created for devices, and using
`udev`

drivers can be automatically loaded for a device.

Rules for `udev`

are located `/etc/udev/rules.d`

.
Any file that ends with `.rules`

in this directory will be parsed when an
event occurs. For more details on how to write rules in these files see
[Writing udev rules](http://www.reactivated.net/writing_udev_rules.html).
For testing, there are utilities such as `udevmonitor`
...
subsystem (connected to `PNP`

bus) and having an id attribute with the value
`PNP0400`

.

When this rule will be found, the command specified under `RUN`

will be
executed to insert the appropriate driver in the kernel.

## Plug and Play[¶](https://linux-kernel-labs.github.io#plug-and-play)
...
The kernel already has implemented most buses using a `bus_type`
...
### PNP bus[¶](https://linux-kernel-labs.github.io#pnp-bus)
...
- used for systems such as serial and parallel ports`ISAPNP`

- offers support for the ISA bus`ACPI`

- offering, among other things, information about system-level devices

The kernel contains a bus, called `pnp_bus`

, that is used for connecting by
many drivers.
...
- driver type associated to the bus`pnp_register_driver()`

- function used to register a PNP driver in the system`pnp_unregister_driver()`

- function used to unregister a PNP driver from the system

As noted in previous sections, the bus has a function called `match`
...
function regarding to the new device. Usually, this
...
Each driver declares and initializes a structure `pnp_driver`
...
### PNP operations[¶](https://linux-kernel-labs.github.io#pnp-operations)
...
### Adding a driver[¶](https://linux-kernel-labs.github.io#adding-a-driver)
...
structure, that the driver must initialize and
register using a function `*_register_driver`

.
For example, for the `PNP`

bus, the driver must declare and initialize a
structure of type `struct pnp_driver`
...
initialization in the init function (`my_init`
...
driver structure.
For example, a driver associated to a `PNP`

bus defines a table of type
`struct pnp_device_id`

and initializes the field `id_table`

from the
structure `pnp_driver my_pnp_driver`
...
defined in the table `my_pnp_tbl`

. This information is used by the bus in
the `match_device`
...
will be created based on the driver name.
Then the bus `match`

function will be called for every supported device,
to associate the driver with any connected device that it supports.

### Removing a driver[¶](https://linux-kernel-labs.github.io#removing-a-driver)

To remove a driver from the kernel, in addition to operations required for a
legacy driver, we must unregister the `device_driver`

structure.
For a driver associated with the `PNP`

bus, we must unregister the `pnp_driver`

structure using the `pnp_unregister_driver()`
...
module unload function (`my_exit`

). When a driver is removed, all the
references to it will be removed for all the devices it supports, and entries
from `sysfs`

will also be removed.

### Adding a new device[¶](https://linux-kernel-labs.github.io#adding-a-new-device)

As we saw above, plug and play drivers do not register devices at initialization.
This operation will take place in the `probe`

function, which is called when
a new device is detected. A device attached to the `PNP`

bus will be added to
the system by the function `probe`
...
device through `hotplug`

), an interrupt is generated and reaches the bus
driver.
The device is registered using the function `device_register()`

and it is
attached to the bus. A call to the user space will also be generated, and the
event can be treated by `udev`

. Then, the list of drivers associated with the
bus is iterated and the `match`

function is called for each of them.
The `match`

function tries to find a driver for the new device. After a
suitable driver is found for the device, the `probe`

function of the driver
is called. If the function ends successfully, the device is added to the driver's
list of devices and new entries are created in `sysfs`

based on the device name.

### Removing a device[¶](https://linux-kernel-labs.github.io#removing-a-device)

As we saw above, the plug and play drivers don't unregister devices when the
driver is unloaded. This operation is done in the `remove`

function, which
is called when a device is removed from the system.
In case of a device attached to the `PNP`

bus, the unregister will be done
in the `remove`

function specified in the `pnp_driver`
...
function is called. A user-space call is also generated, which
can be detected by `udev`

, and entries are removed from `sysfs`
...
and the`pnp_bus_type`

variable.

### 1. Bus implementation[¶](https://linux-kernel-labs.github.io#bus-implementation)

Analyze the contents of the `bex.c`

, a module that implements a bus
...
Load the module and verify that the bus is visible in `/sys/bus`
...
Remove the module and notice that the `sysfs`

entries are removed.

### 2. Add type and version device attributes[¶](https://linux-kernel-labs.github.io#add-type-and-version-device-attributes)

Add two read-only device attributes, `type`
...
### 3. Add del and add bus attributes[¶](https://linux-kernel-labs.github.io#add-del-and-add-bus-attributes)

Add two write-only bus attributes, `del`

and `add`

. del expects the name
...
to parse the input from sysfs and
`bex_del_dev()`
...
The store functions should return `0`
...
. Delete it and observe it disapears from `sysfs`
...
### 4. Register the bex misc driver[¶](https://linux-kernel-labs.github.io#register-the-bex-misc-driver)

Modify **bex-misc.c** so that it registers the driver with the bex
bus. Insert the `bmx_misc.ko`

module and create a new bex device from
...
Notice that the bus match function in **bex.c** is not
implemented.
...
function from the `bex_misc`

driver is called.

### 5. Register misc device in the bex_misc probe function[¶](https://linux-kernel-labs.github.io#register-misc-device-in-the-bex-misc-probe-function)

Modify **bex_misc.c** to refuse probing if `version > 1`

. Also, register the
defined misc device in `bex_misc_probe`

and deregister it in
`bex_misc_remove`

. Follow the **TODO 6** markings.
...
Inspect `/sys/bus/bex/devices/test2`

and observe that we have a new
...
### 6. Monitor uevent notifications[¶](https://linux-kernel-labs.github.io#monitor-uevent-notifications)

Use the `udevadm monitor`

command and observe what happens when:
...
modules are inserted - a new device with the type "type" is created
