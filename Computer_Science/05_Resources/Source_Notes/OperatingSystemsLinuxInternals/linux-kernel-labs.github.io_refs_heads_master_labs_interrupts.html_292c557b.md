Title: I/O access and Interrupts — The Linux Kernel  documentation
Mapped Topic: Kernel labs and lectures
Source URL: https://linux-kernel-labs.github.io/refs/heads/master/labs/interrupts.html
Source Type: official_course_material
Trust Score: 95
Fetched At: 2026-04-17T07:06:16+00:00
Mapped From CSE.md Section: Part 2: C. Operating systems, Linux internals, kernel understanding

# Content

# I/O access and Interrupts[¶](https://linux-kernel-labs.github.io#i-o-access-and-interrupts)
...
## Background information[¶](https://linux-kernel-labs.github.io#background-information)
...
**Control**registers that receive device commands**Status**registers, which contain information about the device's internal status**Input**registers from which data is taken from the device**Output**registers in which the data is written to transmit it to the device
...
## Accessing the hardware[¶](https://linux-kernel-labs.github.io#accessing-the-hardware)

In Linux, the I/O ports access is implemented on all architectures and there are several APIs that can be used.

### Request access to I/O ports[¶](https://linux-kernel-labs.github.io#request-access-to-i-o-ports)
...
### Accessing I/O ports[¶](https://linux-kernel-labs.github.io#accessing-i-o-ports)
...
*unsigned inb(int port)*, reads one byte (8 bits) from port*void outb(unsigned char byte, int port)*, writes one byte (8 bits) to port*unsigned inw(int port)*, reads two bytes (16-bit) ports*void outw(unsigned short word, int port)*, writes two bytes (16-bits) to port*unsigned inl (int port)*, reads four bytes (32-bits) from port*void outl(unsigned long word, int port)*, writes four bytes (32-bits) to port
...
### 5. Accessing I/O ports from userspace[¶](https://linux-kernel-labs.github.io#accessing-i-o-ports-from-userspace)
...
## Interrupt handling[¶](https://linux-kernel-labs.github.io#interrupt-handling)

### Requesting an interrupt[¶](https://linux-kernel-labs.github.io#requesting-an-interrupt)
...
When requesting a shared interrupt (IRQF_SHARED) the
...
### Implementing an interrupt handler[¶](https://linux-kernel-labs.github.io#implementing-an-interrupt-handler)
...
### Interrupt statistics[¶](https://linux-kernel-labs.github.io#interrupt-statistics)
...
### Serial Port[¶](https://linux-kernel-labs.github.io#serial-port)

### Parallel port[¶](https://linux-kernel-labs.github.io#parallel-port)

### Keyboard controller[¶](https://linux-kernel-labs.github.io#keyboard-controller)

[Intel 8042](http://en.wikipedia.org/wiki/Intel_8042)- drivers/input/serio/i8042.c
...
### Keyboard driver[¶](https://linux-kernel-labs.github.io#keyboard-driver)
...
### 1. Request the I/O ports[¶](https://linux-kernel-labs.github.io#request-the-i-o-ports)
...
macro. Also, add code to release the I/O
ports in `kbd_exit()`
...
You can review the [Request access to I/O ports](https://linux-kernel-labs.github.io#request-access-to-i-o-ports) section before
proceeding.
...
Use the function `request_region()`
...
### 2. Interrupt handling routine[¶](https://linux-kernel-labs.github.io#interrupt-handling-routine)

For this task we will implement and register an interrupt handler for
the keyboard interrupt. You can review the [Requesting an interrupt](https://linux-kernel-labs.github.io#requesting-an-interrupt)
...
More details about the format of the */proc/interrupts* can
be found in the [Interrupt statistics](https://linux-kernel-labs.github.io#interrupt-statistics) section.
...
using **dmesg**. Also note that when you use the serial port no
...
### 3. Store ASCII keys to buffer[¶](https://linux-kernel-labs.github.io#store-ascii-keys-to-buffer)
...
#### Reading the data register[¶](https://linux-kernel-labs.github.io#reading-the-data-register)
...
.
Revisit the [Accessing I/O ports](https://linux-kernel-labs.github.io#accessing-i-o-ports) section.
...
#### Interpreting the scancode[¶](https://linux-kernel-labs.github.io#interpreting-the-scancode)
...
As also indicated in this

[article](http://www.linuxjournal.com/article/1080), a key release scancode is 128 (0x80) higher then a key press scancode. This is how we can distinguish between a press key scancode and a release scancode.A scancode is translated into a keycode that matches a key. A pressed scanned keycode and a released scancode have the same keycode. For the keys shown above we have the following table:
...
#### Store characters to the buffer[¶](https://linux-kernel-labs.github.io#store-characters-to-the-buffer)
...
Revisit the [Locking](https://linux-kernel-labs.github.io#locking) section.

### 4. Reading the buffer[¶](https://linux-kernel-labs.github.io#reading-the-buffer)
...
to store it to the user buffer.

Attention

In the read function, use `spin_lock_irqsave()`
...
### 5. Reset the buffer[¶](https://linux-kernel-labs.github.io#reset-the-buffer)
...
Press some keys, then run the command **echo "clear" > /dev/kbd**.
Check the buffer's content again. It should be reset.

## Extra Exercises[¶](https://linux-kernel-labs.github.io#extra-exercises)

### 1. kfifo[¶](https://linux-kernel-labs.github.io#kfifo)

Implement a keylogger using the
[kfifo API](https://elixir.bootlin.com/linux/v4.15/source/include/linux/kfifo.h).

Hint

Follow the [API call examples from the kernel code](https://elixir.bootlin.com/linux/v4.15/source/samples/kfifo).
For example, the file [bytestream-examples.c](https://elixir.bootlin.com/linux/v4.15/source/samples/kfifo/bytestream-example.c).
