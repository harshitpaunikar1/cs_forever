Title: Kernel modules — The Linux Kernel  documentation
Mapped Topic: Kernel labs and lectures
Source URL: https://linux-kernel-labs.github.io/refs/heads/master/labs/kernel_modules.html
Source Type: official_course_material
Trust Score: 95
Fetched At: 2026-04-17T07:06:19+00:00
Mapped From CSE.md Section: Part 2: C. Operating systems, Linux internals, kernel understanding

# Content

# Kernel modules[¶](https://linux-kernel-labs.github.io#kernel-modules)
...
## Kernel Modules Overview[¶](https://linux-kernel-labs.github.io#kernel-modules-overview)
...
## An example of a kernel module[¶](https://linux-kernel-labs.github.io#an-example-of-a-kernel-module)
...
## Compiling kernel modules[¶](https://linux-kernel-labs.github.io#compiling-kernel-modules)
...
module. The easiest example of such a directive is ```
obj-m =
module.o
```
...
sources, resulting in module-a.o and module-b.o objects`module-a.o`

and`module-b.o`

will then be linked in`supermodule.o`

- from
`supermodule.o`

will be created`supermodule.ko`
...
## Loading/unloading a kernel module[¶](https://linux-kernel-labs.github.io#loading-unloading-a-kernel-module)
...
## Kernel Module Debugging[¶](https://linux-kernel-labs.github.io#kernel-module-debugging)
...
generate a [kernel oops](https://en.wikipedia.org/wiki/Linux_kernel_oops).
...
state, a [kernel panic](https://en.wikipedia.org/wiki/Linux_kernel_panic) is
generated.
...
### objdump[¶](https://linux-kernel-labs.github.io#objdump)
...
### addr2line[¶](https://linux-kernel-labs.github.io#addr2line)
...
### minicom[¶](https://linux-kernel-labs.github.io#minicom)
...
### netconsole[¶](https://linux-kernel-labs.github.io#netconsole)
...
### Printk debugging[¶](https://linux-kernel-labs.github.io#printk-debugging)
...
### Dynamic debugging[¶](https://linux-kernel-labs.github.io#dynamic-debugging)

Dynamic [dyndbg](https://www.kernel.org/doc/html/v4.15/admin-guide/dynamic-debug-howto.html)
...
[Debugfs](http://opensourceforu.com/2010/10/debugging-linux-kernel-with-debugfs/)
...
#### Dyndbg Options[¶](https://linux-kernel-labs.github.io#dyndbg-options)
...
includes the thread id if it is not called from interrupt context`_`

no flag is set.

### KDB: Kernel debugger[¶](https://linux-kernel-labs.github.io#kdb-kernel-debugger)
...
gdb and symbol files -- see [the GDB debugging section](https://linux-kernel-labs.github.io/so2/lab1-intro.html#gdb-intro)
...
Using **cscope** or [LXR](http://elixir.free-electrons.com/linux/latest/source) find the definitions of the following symbols
...
- what do the two macros do? What is
`init_module`
...
If you have problems using **cscope**, it is possible that the database
...
When searching for a structure using **cscope**, use only the
structure name (without `struct`

). So, to search for the
...
For more info on using **cscope**, read the
[cscope section](https://linux-kernel-labs.github.io/so2/lab1-intro.html#cscope-intro) in the previous lab.

### 1. Kernel module[¶](https://linux-kernel-labs.github.io#kernel-module)

To work with the kernel modules, we will follow the steps described
[above](https://linux-kernel-labs.github.io/so2/lab1-intro.html#exercises-summary).

- Generate the skeleton for the task named
**1-2-test-mod**then build the module, - by running the following command in
`tools/labs`
...
Read [Loading/unloading a kernel module](https://linux-kernel-labs.github.io#loading-unloading-a-kernel-module) section. When unloading
a kernel module, you can specify only the module name
(without extension).

### 2. Printk[¶](https://linux-kernel-labs.github.io#printk)
...
### 3. Error[¶](https://linux-kernel-labs.github.io#error)
...
### 4. Sub-modules[¶](https://linux-kernel-labs.github.io#sub-modules)
...
Read the [Compiling kernel modules](https://linux-kernel-labs.github.io#compiling-kernel-modules) section of the lab.

Compile, copy, boot the VM, load and unload the kernel module. Make sure messages are properly displayed on the console.

### 5. Kernel oops[¶](https://linux-kernel-labs.github.io#kernel-oops-1)
...
Read [Compiling kernel modules](https://linux-kernel-labs.github.io#compiling-kernel-modules) section of the lab.

Compile the corresponding module and load it into the kernel. Identify the memory address at which the oops appeared.

Hint

Read [`Debugging`_](https://linux-kernel-labs.github.io#system-message-1) section of the lab. To identify the
...
compilation! Read the lab's [objdump](https://linux-kernel-labs.github.io#objdump) and [addr2line](https://linux-kernel-labs.github.io#addr2line)
sections.
...
### 6. Module parameters[¶](https://linux-kernel-labs.github.io#module-parameters)
...
The str variable can be changed by passing a parameter to
the module. Find more information [here](http://tldp.org/LDP/lkmpg/2.6/html/x323.html).

### 7. Proc info[¶](https://linux-kernel-labs.github.io#proc-info-1)
...
. Use[LXR](http://elixir.free-electrons.com/linux/latest/source)or`cscope`

to find the definition of`struct task_struct`
...
variable (of the type`struct task_struct*`
...
### 1. KDB[¶](https://linux-kernel-labs.github.io#kdb)

Go to the **8-kdb** directory. Activate KDB over the serial port and enter KDB
mode using **SysRq**. Connect to the pseudo-terminal linked to virtiocon0
using **minicom**, configure KDB to use the hvc0 serial port:

```
echo hvc0 > /sys/module/kgdboc/parameters/kgdboc
```

and enable it using SysRq (**Ctrl + O g**).
Review the current system status (**help** to see the available KDB
commands). Continue the kernel execution using the **go** command.

Load the `hello_kdb`

module.
The module will simulate a bug when writing to the `/proc/hello_kdb_bug`
...
When writing to `/proc/hello_kdb_break`

, the module will increment the
`kdb_write_address`

variable. Enter KDB and set a breakpoint for each
write access of the `kdb_write_address`
...
### 2. PS Module[¶](https://linux-kernel-labs.github.io#ps-module)

Update the created kernel module at proc-info in order to display
...
result with the output of the **ps** command.
...
macros (such as`for_each_process`

) are useful when you want to navigate the items in a list.- To understand how to use a feature or a macro, use
[LXR](http://elixir.free-electrons.com/linux/latest/source)or Vim and**cscope**and search for usage scenarios.

### 3. Memory Info[¶](https://linux-kernel-labs.github.io#memory-info)
...
and`struct vm_area_struct`

. A memory area is indicated by a structure of type`struct vm_area_struct`

. - Don't forget to include the headers where the necessary structures are defined.

### 4. Dynamic Debugging[¶](https://linux-kernel-labs.github.io#dynamic-debugging-1)

Go to the **9-dyndbg** directory and compile the `dyndbg.ko`

module.

Familiarize yourself with the `debugfs`

file system mounted in
`/debug`

and analyze the contents of the file
`/debug/dynamic_debug/control`

. Insert the `dyndbg.ko`

module and
notice the new content of the `dynamic_debug/control`
...
Configure **dyndbg** so that only messages marked as "Important" in
`my_debug_func()`

function are displayed when the module is unloaded.
The exercise will only filter out the `pr_debug()`
...
Read the [Dynamic debugging](https://linux-kernel-labs.github.io#dynamic-debugging) section and look at the **dyndbg**
options (for example, **line**, **format**).

Perform the filtering and revise the `dynamic_debug/control`
...
Check the **dyndbg** flags. Unload the kernel module and observe the
log messages.

### 5. Dynamic Debugging During Initialization[¶](https://linux-kernel-labs.github.io#dynamic-debugging-during-initialization)

As you have noticed, `pr_debug()`

calls can only be activated /filtered
...
default (fake) parameter called **dyndbg** that can be passed as an
argument to initialize the module. With this parameter you can add /delete
**dyndbg** flags.

Hint

Read the last part of the [Dynamic debugging](https://linux-kernel-labs.github.io#dynamic-debugging) section and see the available
flags (e.g.: **+/- p**).

Read the [Debug Messages section at Module Initialization Time](https://01.org/linuxgraphics/gfx-docs/drm/admin-guide/dynamic-debug-howto.html#debug-messages-at-module-initialization-time)
and insert the module so that the messages in `my_debug_func()`

(called
`dyndbg_init()`

) are also displayed during initialization.

Warning

In the VM from the lab, you will need to use **insmod** instead of
**modprobe**.

Without unloading the module, deactivate `pr_debug()`
