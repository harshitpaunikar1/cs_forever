Title: cpu-sched-mlfq.pdf
Mapped Topic: Operating systems fundamentals
Source URL: https://pages.cs.wisc.edu/~remzi/OSTEP/cpu-sched-mlfq.pdf
Source Type: official_open_book
Trust Score: 97
Fetched At: 2026-04-17T06:54:57+00:00
...
periods of time, MLFQ will reduce its priority . In this way , MLFQwill try
...
3). In the ﬁgure, two jobs (A and B) are at the highest priority le vel, while
...
SCHEDULING :
THE MULTI -L EVEL FEEDBACK QUEUE 3
Q1
Q2
Q3
Q4
...
The allotment is the amount of time a job can spend at a given priorit y
level before the scheduler reduces its priority . For simplicit y , at ﬁrst, we
will assume the allotment is equal to a single time slice.
Here is our ﬁrst attempt at a priority-adjustment algorithm:
...
with the allotment set equal to the time slice). Figure 8.2 show s what
...
(shown in black) is running along in the lowest-priority queue (a s would
any long-running CPU-intensive jobs); B (shown in gray) arrive s at time
...
SCHEDULING :
THE MULTI -L EVEL FEEDBACK QUEUE 5
Q0
Q1
Q2
0 50 100 150 200
...
process gives up the processor before using up its allotment, we ke ep it at
...
the keyboard or mouse), it will relinquish the CPU before its allot ment is
...
teractive job B (shown in gray) that needs the CPU only for 1 ms befor e
...
ity because B keeps releasing the CPU; if B is an interactive j ob, MLFQ
...
SCHEDULING :
THE MULTI -L EVEL FEEDBACK QUEUE
TIP : S CHEDULING MUST BE SECURE FROM ATTACK
...
the following attack: before the allotment is used, issue an I/O op eration
(e.g., to a ﬁle) and thus relinquish the CPU; doing so allows you t o remain
in the same queue, and thus gain a higher percentage of CPU time . When
done right (e.g., by running for 99% of the allotment before relinq uishing
...
in the system. There are many ways to achieve this, but let’s ju st do some-
...
interactive jobs. Two graphs are shown in Figure 8.4 (page 7). O n the left,
there is no priority boost, and thus the long-running job gets star ved once
the two short jobs arrive; on the right, there is a priority boost eve ry 100
...
SCHEDULING :
THE MULTI -L EVEL FEEDBACK QUEUE 7
Q2
Q1
Q0
0 50 100 150 200
...
progress, getting boosted to the highest priority every 100 ms an d thus
...
of the CPU. As such, it is often left to the system administrator t o ﬁnd the
right value – or in the modern world, increasingly , to automatic m ethods
...
Q2
0 50 100 150 200 250 300
Figure 8.5: Without (Left) and With (Right) Gaming T olerance
The solution here is to perform better accounting of CPU time at each
level of the MLFQ. Instead of forgetting how much of its allotment a p ro-
cess used at a given level when it performs I/O, the scheduler s hould
keep track; once a process has used its allotment, it is demoted to the next
priority queue. Whether it uses its allotment in one long burst or m any
small ones should not matter. We thus rewrite Rules 4a and 4b to th e
...
the left) as well the new anti-gaming Rule 4. Without any protec tion from
gaming, a process can issue an I/O before its allotment ends, thu s stay-
ing at the same priority level, and dominating CPU time. With be tter
accounting in place (right), regardless of the I/O behavior of th e process,
it slowly moves down the queues, and thus cannot gain an unfair sha re
of the CPU.
...
vation and account for changes in behavior? There are no easy answ ers
to these questions, and thus only some experience with workloads a nd
...
SCHEDULING :
THE MULTI -L EVEL FEEDBACK QUEUE 9
Q2
Q1
Q0
0 50 100 150 200 250 300
...
ms). Figure 8.6 shows an example in which two jobs run for 20 ms at t he
highest queue (with a 10-ms time slice), 40 ms in the middle (2 0-ms time
...
SCHEDULING :
THE MULTI -L EVEL FEEDBACK QUEUE 11
References
...
classic old books on how a real UNIX operating system is built; a deﬁnite must-read for kernel hackers.
