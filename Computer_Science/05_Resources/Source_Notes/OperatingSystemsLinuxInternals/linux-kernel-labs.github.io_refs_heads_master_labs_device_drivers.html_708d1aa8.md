...
Source URL: https://linux-kernel-labs.github.io/refs/heads/master/labs/device_drivers.html
Source Type: official_course_material
Trust Score: 95
Fetched At: 2026-04-17T07:06:09+00:00
Mapped From CSE.md Section: Part 2: C. Operating systems, Linux internals, kernel understanding

# Content

# Character device drivers[¶](https://linux-kernel-labs.github.io#character-device-drivers)

## Laboratory objectives[¶](https://linux-kernel-labs.github.io#laboratory-objectives)
...
## Majors and minors[¶](https://linux-kernel-labs.github.io#majors-and-minors)
...
command; the command receives the
type (`block`
...
). Thus, if you want to create a character device
named `mycdev`
...
with the major 240 and minor 0
the command will be:
...
## Data structures for a character device[¶](https://linux-kernel-labs.github.io#data-structures-for-a-character-device)

In the kernel, a character-type device is represented by
`struct cdev`

, a structure used to register it in the
...
[¶](https://linux-kernel-labs.github.io#struct-file-operations)
...
, etc. These operations are
described in the fields of the `struct file_operations`
...
does not receive the parameter path or the various parameters that control
the file opening mode. Similarly, `read`
...
parameters two structures: `file`

and `inode`

. Both structures represent a file,
...
is the displacement to be read or written (to be updated accordingly);`user_buffer`
...
are the parameters sent by the users to the ioctl call (IO control).

`inode`

and `file`

structures[¶](https://linux-kernel-labs.github.io#inode-and-file-structures)

An `inode`

represents a file from the point of view of the file system. Attributes
of an inode are the size, rights, times associated with the file. An inode uniquely
identifies a file in a file system.

The `file`

structure is still a file, but closer to the user's point of view.
...
, which specifies the operations associated with the file (pointer to the`file_operations`

structure );`private_data`

, a pointer that can be used by the programmer to store device-specific data; The pointer will be initialized to a memory location assigned by the programmer.`f_pos`
...
## Implementation of operations[¶](https://linux-kernel-labs.github.io#implementation-of-operations)
...
will contain the data associated with a device.
The `cdev`

field (`cdev`

type) is a character-type device and is used to record it
in the system and identify the device. The pointer to the `cdev`

member can be
found using the `i_cdev`
...
stored at open which is then available in the `read`
...
## Registration and unregistration of character devices[¶](https://linux-kernel-labs.github.io#registration-and-unregistration-of-character-devices)

The registration/unregistration of a device is made by specifying the major and
minor. The `dev_t`

type is used to keep the identifiers of a device (both major
and minor) and can be obtained using the `MKDEV`
...
minor (if the max value for minor is exceeded,
...
function must be called only after the device is ready to receive
calls. Removing a device is done using the `cdev_del`
...
Initialization of the struct my_fops used the initialization
...
example, after the initialization above, `my_fops.mmap`

will
be NULL.

## Access to the address space of the process[¶](https://linux-kernel-labs.github.io#access-to-the-address-space-of-the-process)
...
to user-space address`address`

; Type can be one on 8, 16, 32, 64 bit (the maximum supported type depends on the hardware platform);`get_user`

analogue to the previous function, only that val will be set to a value identical to the value at the user-space address given by address;`copy_to_user`

copies`n`

bytes from the kernel-space, from the address referenced by`from`

in user-space to the address referenced by`to`

;`copy_from_user`

copies`n`

bytes from user-space from the address referenced by`from`

in kernel-space to the address referenced by`to`
...
## Open and release[¶](https://linux-kernel-labs.github.io#open-and-release)

The `open`

function performs the initialization of a device. In most cases,
...
), block open calls until a release operation, or shut down
...
## Read and write[¶](https://linux-kernel-labs.github.io#read-and-write)
...
functions transfer data between the device and the
...
why it is necessary to use the `copy_to_user`
...
## ioctl[¶](https://linux-kernel-labs.github.io#ioctl-1)
...
implementing a `ioctl`

function. Initially, the ioctl system call used Big Kernel
Lock. That's why the call was gradually replaced with its unlocked version
called `unlocked_ioctl`

. You can read more on LWN:
...
is the command sent from user-space. If a value is being sent from the
...
function, the numbers corresponding to the
commands must be chosen. One method is to choose consecutive numbers starting
at 0, but it is recommended to use `_IOC(dir, type, nr, size)`
...
## Waiting queues[¶](https://linux-kernel-labs.github.io#waiting-queues)
...
event. A queue is defined with the `wait_queue_head_t`
...
initializes the queue; to initialize the queue at compile time, you can use the`DECLARE_WAIT_QUEUE_HEAD`

macro;`wait_event()`

and`wait_event_interruptible()`

adds the current thread to the queue while the condition is false, sets it to TASK_UNINTERRUPTIBLE or TASK_INTERRUPTIBLE and calls the scheduler to schedule a new thread; Waiting will be interrupted when another thread will call the wake_up function;`wait_event_timeout()`

and`wait_event_interruptible_timeout()`

have the same effect as the above functions, only waiting can be interrupted at the end of the timeout received as a parameter;`wake_up()`

puts all threads off from state TASK_INTERRUPTIBLE and TASK_UNINTERRUPTIBLE in TASK_RUNNING status; Remove these threads from the queue;`wake_up_interruptible()`

same action, but only threads with TASK_INTERRUPTIBLE status are woken up.
...
### 0. Intro[¶](https://linux-kernel-labs.github.io#intro)

Using [LXR](http://elixir.free-electrons.com/linux/latest/source) find the definitions
...
### 1. Register/unregister[¶](https://linux-kernel-labs.github.io#register-unregister)
...
/dev/so2_cdevcharacter device node usingmknod.Hint

Read

[Majors and minors]section in the lab.Implement the registration and deregistration of the device with the name

`so2_cdev`

, respectively in the init and exit module functions. ImplementTODO 1.Hint

Read the section

[Registration and unregistration of character devices]Display, using

`pr_info`

, a message after the registration and unregistration operations to confirm that they were successful. Then load the module into the kernel:$ insmod so2_cdev.koAnd see character devices in
...
### 2. Register an already registered major[¶](https://linux-kernel-labs.github.io#register-an-already-registered-major)
...
See [errno-base.h](http://elixir.free-electrons.com/linux/v4.9/source/include/uapi/asm-generic/errno-base.h)
and figure out what does the error code mean.
Return to the initial configuration of the module.

### 3. Open and close[¶](https://linux-kernel-labs.github.io#open-and-close)

Run `cat /dev/so2_cdev`

to read data from our char device.
...
structure.- Read the section
[Registration and unregistration of character devices]in the lab.- Implement the open and release functions in the driver.
- Display a message in the open and release functions.
- Read again
`/dev/so2_cdev`

file. Follow the messages displayed by the kernel. We still get an error because`read`
...
structure. Read [Open and release](https://linux-kernel-labs.github.io#open-and-release) section.

### 4. Access restriction[¶](https://linux-kernel-labs.github.io#access-restriction)
...
). Restricting access will be done in the open function displayed by
...
- To test your deployment, you'll need to simulate a long-term use of your device. To simulate a sleep, call the scheduler at the end of the device opening:
...
The advantage of the atomic_cmpxchg function is that it can check the
old value of the variable and set it up to a new value, all in one
atomic operation. Read more details about [atomic_cmpxchg](https://www.khronos.org/registry/OpenCL/sdk/1.1/docs/man/xhtml/atomic_cmpxchg.html)
An example of use is [here](http://elixir.free-electrons.com/linux/v4.9/source/lib/dump_stack.c#L24).

### 5. Read operation[¶](https://linux-kernel-labs.github.io#read-operation)

Implement the read function in the driver. Follow comments marked with `TODO 4`
...
structure initialized with the value of`MESSAGE`

macro. Initializing this buffer will be done in module`init`

function.- At a read call, copy the contents of the kernel space buffer into the user space buffer.

- Use the
`copy_to_user()`

function to copy information from kernel space to user space.- Ignore the size and offset parameters at this time. You can assume that the buffer in user space is large enough. You do not need to check the validity of the size argument of the read function.
...
does not end (use Ctrl+C).
Read the [read and write](https://linux-kernel-labs.github.io#read-and-write) sections and [Access to the address space of the process](https://linux-kernel-labs.github.io#access-to-the-address-space-of-the-process)
...
command reads to the end of the file, and the end of the file is
...
- Ensure that the read function returns the number of bytes that were copied into the user buffer.
...
### 6. Write operation[¶](https://linux-kernel-labs.github.io#write-operation)
...
### 7. ioctl operation[¶](https://linux-kernel-labs.github.io#ioctl-operation)

For this exercise, we want to add the ioctl `MY_IOCTL_PRINT`

to display the
message from the `IOCTL_MESSAGE`
...
to call the ioctl function with the appropriate parameters.- To test, we will use an user-space program (
`user/so2_cdev_test.c`

) which will call the`ioctl`
...
Read the [ioctl](https://linux-kernel-labs.github.io#ioctl) section in the lab.

Note

The user-space code is compiled automatically at `make build`
...
if your host is 64 bit then you need to install `gcc-multilib`

package.

## Extra Exercises[¶](https://linux-kernel-labs.github.io#extra-exercises)

### Ioctl with messaging[¶](https://linux-kernel-labs.github.io#ioctl-with-messaging)
...
function from the driver the following operations:

`MY_IOCTL_SET_BUFFER`

for writing a message to the device;`MY_IOCTL_GET_BUFFER`

to read a message from your device.- For testing, pass the required command line arguments to the user-space program.

Note

Read the [ioctl](https://linux-kernel-labs.github.io#ioctl) and [Access to the address space of the process](https://linux-kernel-labs.github.io#access-to-the-address-space-of-the-process)
sections of the lab.

### Ioctl with waiting queues[¶](https://linux-kernel-labs.github.io#ioctl-with-waiting-queues)
...
to remove the process from a queue.- Fill the device structure with a
`wait_queue_head_t`
...
Read the [ioctl](https://linux-kernel-labs.github.io#ioctl) and [Waiting queues](https://linux-kernel-labs.github.io#waiting-queues) sections in the lab.

### O_NONBLOCK implementation[¶](https://linux-kernel-labs.github.io#o-nonblock-implementation)
...
, the read call will return`-EWOULDBLOCK`
...
, to allow testing. - For testing, pass the required command line arguments to the
user-space program.
- when using the
`n`

option, the test program will change the open flags to`O_NONBLOCK`
