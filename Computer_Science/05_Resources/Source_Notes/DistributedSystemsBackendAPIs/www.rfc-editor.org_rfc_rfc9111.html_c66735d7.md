...
Mapped Topic: HTTP caching
Source URL: https://www.rfc-editor.org/rfc/rfc9111.html
Source Type: official_standard
Trust Score: 99
Fetched At: 2026-04-17T07:08:10+00:00
...
control cache behavior or indicate cacheable response messages.[¶](https://www.rfc-editor.org#section-abstract-1)

This document obsoletes RFC 7234.[¶](https://www.rfc-editor.org#section-abstract-2)
...
[https://www.rfc-editor.org/info/rfc9111](https://www.rfc-editor.org/info/rfc9111).[¶](https://www.rfc-editor.org#section-boilerplate.1-3)
...
warranty as described in the Revised BSD License.[¶](https://www.rfc-editor.org#section-boilerplate.2-2)
...
into languages other than English.[¶](https://www.rfc-editor.org#section-boilerplate.2-3)
...
messages.[¶](https://www.rfc-editor.org#section-1-1)
...
server MAY use a cache, though not when acting as a tunnel ([Section 3.7](https://www.rfc-editor.org/rfc/rfc9110#section-3.7) of [[HTTP](https://www.rfc-editor.org#HTTP)]).[¶](https://www.rfc-editor.org#section-1-2)
...
[Section 4.2](https://www.rfc-editor.org#expiration.model), if it can be reused without
...
can freshen it ([Section 4.3](https://www.rfc-editor.org#validation.model)) or if the
origin is unavailable ([Section 4.2.4](https://www.rfc-editor.org#serving.stale.responses)).[¶](https://www.rfc-editor.org#section-1-4)

This document obsoletes [RFC 7234](https://www.rfc-editor.org#RFC7234),
with the changes being summarized in [Appendix B](https://www.rfc-editor.org#changes.from.rfc.7234).[¶](https://www.rfc-editor.org#section-1-5)

The key words "MUST", "MUST NOT",
"REQUIRED", "SHALL", "SHALL NOT", "SHOULD", "SHOULD NOT",
"RECOMMENDED", "NOT RECOMMENDED",
"MAY", and "OPTIONAL" in this document are
to be interpreted as described in BCP 14 [[RFC2119](https://www.rfc-editor.org#RFC2119)]
[[RFC8174](https://www.rfc-editor.org#RFC8174)] when, and only when, they appear in all capitals,
as shown here.[¶](https://www.rfc-editor.org#section-1.1-1)

[Section 2](https://www.rfc-editor.org/rfc/rfc9110#section-2) of [[HTTP](https://www.rfc-editor.org#HTTP)] defines conformance criteria and contains considerations regarding error handling.[¶](https://www.rfc-editor.org#section-1.1-2)

This specification uses the Augmented Backus-Naur Form (ABNF) notation of
[[RFC5234](https://www.rfc-editor.org#RFC5234)], extended with the notation for case-sensitivity
in strings defined in [[RFC7405](https://www.rfc-editor.org#RFC7405)].[¶](https://www.rfc-editor.org#section-1.2-1)

It also uses a list extension, defined in [Section 5.6.1](https://www.rfc-editor.org/rfc/rfc9110#section-5.6.1) of [[HTTP](https://www.rfc-editor.org#HTTP)],
that allows for compact definition of comma-separated lists using a "#"
operator (similar to how the "*" operator indicates repetition). [Appendix A](https://www.rfc-editor.org#collected.abnf) shows the collected grammar with all list
operators expanded to standard ABNF notation.[¶](https://www.rfc-editor.org#section-1.2-2)

The following core rule is included by
reference, as defined in [[RFC5234](https://www.rfc-editor.org#RFC5234)], [Appendix B.1](https://www.rfc-editor.org/rfc/rfc5234#appendix-B.1):
DIGIT (decimal 0-9).[¶](https://www.rfc-editor.org#section-1.2.1-1)
...
in seconds.[¶](https://www.rfc-editor.org#section-1.2.2-1)
...
(231) or the greatest positive integer it can conveniently
represent.[¶](https://www.rfc-editor.org#section-1.2.2-3)

Proper cache operation preserves the semantics of HTTP transfers
while reducing the transmission of information already held in the
cache. See [Section 3](https://www.rfc-editor.org/rfc/rfc9110#section-3) of [[HTTP](https://www.rfc-editor.org#HTTP)]
for the general terminology and core concepts of HTTP.[¶](https://www.rfc-editor.org#section-2-1)
...
caches always store and reuse particular responses.[¶](https://www.rfc-editor.org#section-2-2)
...
use the URI as the cache key.[¶](https://www.rfc-editor.org#section-2-3)
...
response header field, as per [Section 4.1](https://www.rfc-editor.org#caching.negotiated.responses).[¶](https://www.rfc-editor.org#section-2-4)
...
thereby "double keying" the cache to avoid some privacy risks (see [Section 7.2](https://www.rfc-editor.org#security.timing)).[¶](https://www.rfc-editor.org#section-2-5)
...
([Section 9.3.1](https://www.rfc-editor.org/rfc/rfc9110#section-9.3.1) of [[HTTP](https://www.rfc-editor.org#HTTP)]). However, it is also possible to store
...
caching and defines something suitable for use as a cache key.[¶](https://www.rfc-editor.org#section-2-6)

A cache is "disconnected" when it cannot contact the origin
server or otherwise find a forward path for a request. A
disconnected cache can serve stale responses in some circumstances ([Section 4.2.4](https://www.rfc-editor.org#serving.stale.responses)).[¶](https://www.rfc-editor.org#section-2-7)

A cache MUST NOT store a response to a request unless:[¶](https://www.rfc-editor.org#section-3-1)

-
the request method is understood by the cache;

[¶](https://www.rfc-editor.org#section-3-2.1.1) -
the response status code is final (see

[Section 15](https://www.rfc-editor.org/rfc/rfc9110#section-15)of [[HTTP](https://www.rfc-editor.org#HTTP)]);[¶](https://www.rfc-editor.org#section-3-2.2.1) -
if the response status code is 206 or 304, or the must-understand cache directive (see

[Section 5.2.2.3](https://www.rfc-editor.org#cache-response-directive.must-understand)) is present: the cache understands the response status code;[¶](https://www.rfc-editor.org#section-3-2.3.1) -
the no-store cache directive is not present in the response (see

[Section 5.2.2.5](https://www.rfc-editor.org#cache-response-directive.no-store));[¶](https://www.rfc-editor.org#section-3-2.4.1) -
if the cache is shared: the private response directive is either not present or allows a shared cache to store a modified response; see

[Section 5.2.2.7](https://www.rfc-editor.org#cache-response-directive.private));[¶](https://www.rfc-editor.org#section-3-2.5.1) -
if the cache is shared: the Authorization header field is not present in the request (see

[Section 11.6.2](https://www.rfc-editor.org/rfc/rfc9110#section-11.6.2)of [[HTTP](https://www.rfc-editor.org#HTTP)]) or a response directive is present that explicitly allows shared caching (see[Section 3.5](https://www.rfc-editor.org#caching.authenticated.responses)); and[¶](https://www.rfc-editor.org#section-3-2.6.1) -
the response contains at least one of the following:

[¶](https://www.rfc-editor.org#section-3-2.7.1)- a public response directive
(see
[Section 5.2.2.9](https://www.rfc-editor.org#cache-response-directive.public));[¶](https://www.rfc-editor.org#section-3-2.7.2.1) - a private response directive, if the cache is not shared
(see
[Section 5.2.2.7](https://www.rfc-editor.org#cache-response-directive.private));[¶](https://www.rfc-editor.org#section-3-2.7.2.2) - an
[Expires](https://www.rfc-editor.org#field.expires)header field (see[Section 5.3](https://www.rfc-editor.org#field.expires));[¶](https://www.rfc-editor.org#section-3-2.7.2.3) - a max-age response directive
(see
[Section 5.2.2.1](https://www.rfc-editor.org#cache-response-directive.max-age));[¶](https://www.rfc-editor.org#section-3-2.7.2.4) - if the cache is shared: an s-maxage response directive
(see
[Section 5.2.2.10](https://www.rfc-editor.org#cache-response-directive.s-maxage));[¶](https://www.rfc-editor.org#section-3-2.7.2.5) - a cache extension that allows it to be cached
(see
[Section 5.2.3](https://www.rfc-editor.org#cache.control.extensions)); or[¶](https://www.rfc-editor.org#section-3-2.7.2.6) - a status code that is defined as heuristically cacheable
(see
[Section 4.2.2](https://www.rfc-editor.org#heuristic.freshness)).[¶](https://www.rfc-editor.org#section-3-2.7.2.7)

- a public response directive
(see

Note that a cache extension can override any of the requirements
listed; see [Section 5.2.3](https://www.rfc-editor.org#cache.control.extensions).[¶](https://www.rfc-editor.org#section-3-3)
...
caching-related behavior.[¶](https://www.rfc-editor.org#section-3-4)
...
prohibited from storing such responses.[¶](https://www.rfc-editor.org#section-3-5)
...
are made:[¶](https://www.rfc-editor.org#section-3.1-1)

- The Connection header field and fields whose names are listed in it are
required by
[Section 7.6.1](https://www.rfc-editor.org/rfc/rfc9110#section-7.6.1)of [[HTTP](https://www.rfc-editor.org#HTTP)] to be removed before forwarding the message. This MAY be implemented by doing so before storage.[¶](https://www.rfc-editor.org#section-3.1-2.1) - Likewise, some fields' semantics require them to be removed
before forwarding the message, and this MAY be implemented by doing so
before storage; see
[Section 7.6.1](https://www.rfc-editor.org/rfc/rfc9110#section-7.6.1)of [[HTTP](https://www.rfc-editor.org#HTTP)] for some examples.[¶](https://www.rfc-editor.org#section-3.1-2.2) - The no-cache (
[Section 5.2.2.4](https://www.rfc-editor.org#cache-response-directive.no-cache)) and private ([Section 5.2.2.7](https://www.rfc-editor.org#cache-response-directive.private)) cache directives can have arguments that prevent storage of header fields by all caches and shared caches, respectively.[¶](https://www.rfc-editor.org#section-3.1-2.3) - Header fields that are specific to the proxy that a cache uses when forwarding a request
...
(
[Section 11.7.1](https://www.rfc-editor.org/rfc/rfc9110#section-11.7.1)of [[HTTP](https://www.rfc-editor.org#HTTP)]), Proxy-Authentication-Info ([Section 11.7.3](https://www.rfc-editor.org/rfc/rfc9110#section-11.7.3)of [[HTTP](https://www.rfc-editor.org#HTTP)]), and Proxy-Authorization ([Section 11.7.2](https://www.rfc-editor.org/rfc/rfc9110#section-11.7.2)of [[HTTP](https://www.rfc-editor.org#HTTP)]).[¶](https://www.rfc-editor.org#section-3.1-2.4)

Caches MAY either store trailer fields separate from header fields or
discard them. Caches MUST NOT combine trailer fields with header fields.[¶](https://www.rfc-editor.org#section-3.1-3)

Caches are required to update a stored response's header fields from another
(typically newer) response in several situations; for example, see Sections [3.4](https://www.rfc-editor.org#combining.responses), [4.3.4](https://www.rfc-editor.org#freshening.responses), and
[4.3.5](https://www.rfc-editor.org#head.effects).[¶](https://www.rfc-editor.org#section-3.2-1)
...
with the following exceptions:[¶](https://www.rfc-editor.org#section-3.2-2)

- Header fields excepted from storage in
[Section 3.1](https://www.rfc-editor.org#storing.fields),[¶](https://www.rfc-editor.org#section-3.2-3.1) - Header fields that the cache's stored response depends upon, as described below,
[¶](https://www.rfc-editor.org#section-3.2-3.2) - Header fields that are automatically processed and removed by the recipient, as described below, and
[¶](https://www.rfc-editor.org#section-3.2-3.3) - The Content-Length header field.
[¶](https://www.rfc-editor.org#section-3.2-3.4)
...
necessary to assure integrity of the stored response.[¶](https://www.rfc-editor.org#section-3.2-4)
...
now be invalid.[¶](https://www.rfc-editor.org#section-3.2-5)
...
even when the processing does not actually occur.[¶](https://www.rfc-editor.org#section-3.2-6)

Note that the Content-* prefix is not a signal that a header field is omitted
from update; it is a convention for MIME header fields, not HTTP.[¶](https://www.rfc-editor.org#section-3.2-7)
...
cache MAY store a response that is not complete ([Section 6.1](https://www.rfc-editor.org/rfc/rfc9110#section-6.1) of [[HTTP](https://www.rfc-editor.org#HTTP)]) provided that the stored response
is recorded as being incomplete. Likewise, a 206 (Partial
Content) response MAY be stored as if it were an incomplete
200 (OK) response. However, a cache MUST NOT store
incomplete or partial-content responses if it does not support the
Range and Content-Range header fields or if
it does not understand the range units used in those fields.[¶](https://www.rfc-editor.org#section-3.3-1)

A cache MAY complete a stored incomplete response by making a subsequent
range request ([Section 14.2](https://www.rfc-editor.org/rfc/rfc9110#section-14.2) of [[HTTP](https://www.rfc-editor.org#HTTP)]) and combining the successful response with the
stored response, as defined in [Section 3.4](https://www.rfc-editor.org#combining.responses). A cache
...
using the 206 (Partial Content) status code.[¶](https://www.rfc-editor.org#section-3.3-2)

A response might transfer only a partial representation if the
connection closed prematurely or if the request used one or more Range
specifiers ([Section 14.2](https://www.rfc-editor.org/rfc/rfc9110#section-14.2) of [[HTTP](https://www.rfc-editor.org#HTTP)]). After several such transfers, a cache might have
...
the cache complies with the client requirements in [Section 15.3.7.3](https://www.rfc-editor.org/rfc/rfc9110#section-15.3.7.3) of [[HTTP](https://www.rfc-editor.org#HTTP)].[¶](https://www.rfc-editor.org#section-3.4-1)
...
provided in the new response, as per [Section 3.2](https://www.rfc-editor.org#update).[¶](https://www.rfc-editor.org#section-3.4-2)

A shared cache MUST NOT use a cached response to a request with an
Authorization header field ([Section 11.6.2](https://www.rfc-editor.org/rfc/rfc9110#section-11.6.2) of [[HTTP](https://www.rfc-editor.org#HTTP)]) to
satisfy any subsequent request unless the response contains a
[Cache-Control](https://www.rfc-editor.org#field.cache-control) field with a response directive
([Section 5.2.2](https://www.rfc-editor.org#cache-response-directive)) that allows it to be stored by
a shared cache, and the cache conforms to the requirements of that
directive for that response.[¶](https://www.rfc-editor.org#section-3.5-1)

In this specification, the following response directives have such an effect:
must-revalidate ([Section 5.2.2.2](https://www.rfc-editor.org#cache-response-directive.must-revalidate)),
public ([Section 5.2.2.9](https://www.rfc-editor.org#cache-response-directive.public)), and
s-maxage ([Section 5.2.2.10](https://www.rfc-editor.org#cache-response-directive.s-maxage)).[¶](https://www.rfc-editor.org#section-3.5-2)

When presented with a request, a cache MUST NOT reuse a stored response
unless:[¶](https://www.rfc-editor.org#section-4-1)

-
the presented target URI (

[Section 7.1](https://www.rfc-editor.org/rfc/rfc9110#section-7.1)of [[HTTP](https://www.rfc-editor.org#HTTP)]) and that of the stored response match, and[¶](https://www.rfc-editor.org#section-4-2.1.1) -
the request method associated with the stored response allows it to be used for the presented request, and

[¶](https://www.rfc-editor.org#section-4-2.2.1) -
request header fields nominated by the stored response (if any) match those presented (see

[Section 4.1](https://www.rfc-editor.org#caching.negotiated.responses)), and[¶](https://www.rfc-editor.org#section-4-2.3.1) -
the stored response does not contain the no-cache directive (

[Section 5.2.2.4](https://www.rfc-editor.org#cache-response-directive.no-cache)), unless it is successfully validated ([Section 4.3](https://www.rfc-editor.org#validation.model)), and[¶](https://www.rfc-editor.org#section-4-2.4.1) -
the stored response is one of the following:

[¶](https://www.rfc-editor.org#section-4-2.5.1)- fresh (see
[Section 4.2](https://www.rfc-editor.org#expiration.model)), or[¶](https://www.rfc-editor.org#section-4-2.5.2.1) - allowed to be served stale (see
[Section 4.2.4](https://www.rfc-editor.org#serving.stale.responses)), or[¶](https://www.rfc-editor.org#section-4-2.5.2.2) - successfully validated (see
[Section 4.3](https://www.rfc-editor.org#validation.model)).[¶](https://www.rfc-editor.org#section-4-2.5.2.3)

- fresh (see

Note that a cache extension can override any of the requirements
listed; see [Section 5.2.3](https://www.rfc-editor.org#cache.control.extensions).[¶](https://www.rfc-editor.org#section-4-3)

When a stored response is used to satisfy a request without validation, a
cache MUST generate an [Age](https://www.rfc-editor.org#field.age) header field ([Section 5.1](https://www.rfc-editor.org#field.age)), replacing any present in the response with a value
equal to the stored response's current_age; see [Section 4.2.3](https://www.rfc-editor.org#age.calculations).[¶](https://www.rfc-editor.org#section-4-4)

A cache MUST write through requests with methods that are unsafe
([Section 9.2.1](https://www.rfc-editor.org/rfc/rfc9110#section-9.2.1) of [[HTTP](https://www.rfc-editor.org#HTTP)]) to the origin server; i.e., a cache is not allowed to
generate a reply to such a request before having forwarded the request and
having received a corresponding response.[¶](https://www.rfc-editor.org#section-4-5)

Also, note that unsafe requests might invalidate already-stored responses;
see [Section 4.4](https://www.rfc-editor.org#invalidation).[¶](https://www.rfc-editor.org#section-4-6)
...
additional latency.[¶](https://www.rfc-editor.org#section-4-7)
...
"Cache-Control: no-cache" to disambiguate which response to use.[¶](https://www.rfc-editor.org#section-4-8)

A cache without a clock ([Section 5.6.7](https://www.rfc-editor.org/rfc/rfc9110#section-5.6.7) of [[HTTP](https://www.rfc-editor.org#HTTP)]) MUST revalidate
stored responses upon every use.[¶](https://www.rfc-editor.org#section-4-9)
...
([Section 12.5.5](https://www.rfc-editor.org/rfc/rfc9110#section-12.5.5) of [[HTTP](https://www.rfc-editor.org#HTTP)]),
...
request that caused the cached response to be stored).[¶](https://www.rfc-editor.org#section-4.1-1)
...
second request by applying any of the following:[¶](https://www.rfc-editor.org#section-4.1-2)

-
adding or removing whitespace, where allowed in the header field's
syntax
[¶](https://www.rfc-editor.org#section-4.1-3.1) -
combining multiple header field lines with the same field name
(see
[Section 5.2](https://www.rfc-editor.org/rfc/rfc9110#section-5.2)of [[HTTP](https://www.rfc-editor.org#HTTP)])[¶](https://www.rfc-editor.org#section-4.1-3.2) -
...
[¶](https://www.rfc-editor.org#section-4.1-3.3)
...
there.[¶](https://www.rfc-editor.org#section-4.1-4)

A stored response with a Vary header field value containing
a member "*" always fails to match.[¶](https://www.rfc-editor.org#section-4.1-5)
...
chosen, as
per [Section 4](https://www.rfc-editor.org#constructing.responses.from.caches).[¶](https://www.rfc-editor.org#section-4.1-6)
...
(see [Section 4.2.3](https://www.rfc-editor.org#age.calculations)) stored response with a valid Vary
field value.[¶](https://www.rfc-editor.org#section-4.1-7)
...
has already stored ([Section 4.3](https://www.rfc-editor.org#validation.model)).[¶](https://www.rfc-editor.org#section-4.1-8)

A "fresh" response is one whose age has not yet exceeded its
freshness lifetime. Conversely, a "stale" response is one where it has.[¶](https://www.rfc-editor.org#section-4.2-1)
...
available.[¶](https://www.rfc-editor.org#section-4.2-2)

A response's "age" is the time that has passed since it was
generated by, or successfully validated with, the origin server.[¶](https://www.rfc-editor.org#section-4.2-3)
...
efficiency.[¶](https://www.rfc-editor.org#section-4.2-4)
...
[Expires](https://www.rfc-editor.org#field.expires) header field ([Section 5.3](https://www.rfc-editor.org#field.expires)) or
the max-age response directive ([Section 5.2.2.1](https://www.rfc-editor.org#cache-response-directive.max-age)). Generally, origin servers
...
significant way before the expiration time is reached.[¶](https://www.rfc-editor.org#section-4.2-5)
...
cached response before reusing it for subsequent requests (see [Section 4.2.4](https://www.rfc-editor.org#serving.stale.responses)).[¶](https://www.rfc-editor.org#section-4.2-6)

Since origin servers do not always provide explicit expiration times,
caches are also allowed to use a heuristic to determine an expiration time
under certain circumstances (see [Section 4.2.2](https://www.rfc-editor.org#heuristic.freshness)).[¶](https://www.rfc-editor.org#section-4.2-7)

The calculation to determine if a response is fresh is:[¶](https://www.rfc-editor.org#section-4.2-8)

response_is_fresh = (freshness_lifetime > current_age)

freshness_lifetime is defined in [Section 4.2.1](https://www.rfc-editor.org#calculating.freshness.lifetime); current_age is defined in
[Section 4.2.3](https://www.rfc-editor.org#age.calculations).[¶](https://www.rfc-editor.org#section-4.2-10)

Clients can send the max-age or min-fresh request directives ([Section 5.2.1](https://www.rfc-editor.org#cache-request-directive)) to suggest limits on the freshness
calculations for the corresponding response. However, caches are not
required to honor them.[¶](https://www.rfc-editor.org#section-4.2-11)

When calculating freshness, to avoid common problems in date parsing:[¶](https://www.rfc-editor.org#section-4.2-12)

- Although all date formats are specified to be case-sensitive,
a cache recipient SHOULD match the field value
case-insensitively.
[¶](https://www.rfc-editor.org#section-4.2-13.1) - If a cache recipient's internal implementation of time has less
resolution than the value of an HTTP-date, the recipient MUST
internally represent a parsed
[Expires](https://www.rfc-editor.org#field.expires)date as the nearest time equal to or earlier than the received value.[¶](https://www.rfc-editor.org#section-4.2-13.2) - A cache recipient MUST NOT allow local time zones to influence the
calculation or comparison of an age or expiration time.
[¶](https://www.rfc-editor.org#section-4.2-13.3) - A cache recipient SHOULD consider a date with a zone abbreviation
other than "GMT" to be invalid for calculating expiration.
[¶](https://www.rfc-editor.org#section-4.2-13.4)

Note that freshness applies only to cache operation; it cannot be used to
force a user agent to refresh its display or reload a resource. See [Section 6](https://www.rfc-editor.org#history.lists) for an explanation of the difference between
caches and history mechanisms.[¶](https://www.rfc-editor.org#section-4.2-14)

A cache can calculate the freshness lifetime (denoted as
freshness_lifetime) of a response by evaluating the following rules and using the first match:[¶](https://www.rfc-editor.org#section-4.2.1-1)

- If the cache is shared and the s-maxage response directive
(
[Section 5.2.2.10](https://www.rfc-editor.org#cache-response-directive.s-maxage)) is present, use its value, or[¶](https://www.rfc-editor.org#section-4.2.1-2.1) - If the max-age response directive (
[Section 5.2.2.1](https://www.rfc-editor.org#cache-response-directive.max-age)) is present, use its value, or[¶](https://www.rfc-editor.org#section-4.2.1-2.2) - If the
[Expires](https://www.rfc-editor.org#field.expires)response header field ([Section 5.3](https://www.rfc-editor.org#field.expires)) is present, use its value minus the value of the Date response header field (using the time the message was received if it is not present, as per[Section 6.6.1](https://www.rfc-editor.org/rfc/rfc9110#section-6.6.1)of [[HTTP](https://www.rfc-editor.org#HTTP)]), or[¶](https://www.rfc-editor.org#section-4.2.1-2.3) - Otherwise, no explicit expiration time is present in the response. A
heuristic freshness lifetime might be applicable; see
[Section 4.2.2](https://www.rfc-editor.org#heuristic.freshness).[¶](https://www.rfc-editor.org#section-4.2.1-2.4)

Note that this calculation is intended to reduce clock skew by using the
clock information provided by the origin server whenever possible.[¶](https://www.rfc-editor.org#section-4.2.1-3)

When there is more than one value present for a given directive (e.g., two
[Expires](https://www.rfc-editor.org#field.expires) header field lines or multiple Cache-Control: max-age
...
be stale.[¶](https://www.rfc-editor.org#section-4.2.1-4)
...
impose worst-case constraints on their results.[¶](https://www.rfc-editor.org#section-4.2.2-1)
...
requirements in [Section 3](https://www.rfc-editor.org#response.cacheability),
...
[Section 15.1](https://www.rfc-editor.org/rfc/rfc9110#section-15.1) of [[HTTP](https://www.rfc-editor.org#HTTP)]) and on responses without
explicit freshness that have been marked as explicitly cacheable (e.g.,
with a public response directive).[¶](https://www.rfc-editor.org#section-4.2.2-2)

Note that in previous specifications, heuristically cacheable response status
codes were called "cacheable by default".[¶](https://www.rfc-editor.org#section-4.2.2-3)

If the response has a Last-Modified header field
([Section 8.8.2](https://www.rfc-editor.org/rfc/rfc9110#section-8.8.2) of [[HTTP](https://www.rfc-editor.org#HTTP)]), caches are encouraged to use a heuristic
expiration value that is no more than some fraction of the interval since
that time. A typical setting of this fraction might be 10%.[¶](https://www.rfc-editor.org#section-4.2.2-4)

The [Age](https://www.rfc-editor.org#field.age) header field is used to convey an estimated
...
has been in transit along network paths.[¶](https://www.rfc-editor.org#section-4.2.3-1)

Age calculation uses the following data:[¶](https://www.rfc-editor.org#section-4.2.3-2)

- "age_value"
-
The term "age_value" denotes the value of the
[Age](https://www.rfc-editor.org#field.age)header field ([Section 5.1](https://www.rfc-editor.org#field.age)), in a form appropriate for arithmetic operation; or 0, if not available.[¶](https://www.rfc-editor.org#section-4.2.3-3.2) - "date_value"
-
The term "date_value" denotes the value of
the Date header field, in a form appropriate for arithmetic
operations. See
[Section 6.6.1](https://www.rfc-editor.org/rfc/rfc9110#section-6.6.1)of [[HTTP](https://www.rfc-editor.org#HTTP)] for the definition of the Date header field and for requirements regarding responses without it.[¶](https://www.rfc-editor.org#section-4.2.3-3.4) - "now"
-
The term "now" means the current value of this implementation's clock
(
[Section 5.6.7](https://www.rfc-editor.org/rfc/rfc9110#section-5.6.7)of [[HTTP](https://www.rfc-editor.org#HTTP)]).[¶](https://www.rfc-editor.org#section-4.2.3-3.6) - "request_time"
-
The value of the clock at the time of the request that
resulted in the stored response.
[¶](https://www.rfc-editor.org#section-4.2.3-3.8) - "response_time"
-
The value of the clock at the time the response
was received.
[¶](https://www.rfc-editor.org#section-4.2.3-3.10)

A response's age can be calculated in two entirely independent ways:[¶](https://www.rfc-editor.org#section-4.2.3-4)
...
[¶](https://www.rfc-editor.org#section-4.2.3-5.1) - the "corrected_age_value", if all of the caches along the response
...
[¶](https://www.rfc-editor.org#section-4.2.3-5.2)
...
insert [Age](https://www.rfc-editor.org#field.age) are present, corrected_initial_age can be calculated
more conservatively as[¶](https://www.rfc-editor.org#section-4.2.3-7)
...
the origin server to the corrected_initial_age.[¶](https://www.rfc-editor.org#section-4.2.3-9)
...
the calculations in [Section 4.2](https://www.rfc-editor.org#expiration.model).[¶](https://www.rfc-editor.org#section-4.2.4-1)
...
s-maxage or proxy-revalidate response directive; see [Section 5.2.2](https://www.rfc-editor.org#cache-response-directive)).[¶](https://www.rfc-editor.org#section-4.2.4-2)
...
(e.g., by the max-stale request directive in [Section 5.2.1](https://www.rfc-editor.org#cache-request-directive), extension directives such as those
defined in [[RFC5861](https://www.rfc-editor.org#RFC5861)], or configuration in accordance
with an out-of-band contract).[¶](https://www.rfc-editor.org#section-4.2.4-3)
...
be chosen; see [Section 4.1](https://www.rfc-editor.org#caching.negotiated.responses)), it can use
the conditional request mechanism ([Section 13](https://www.rfc-editor.org/rfc/rfc9110#section-13) of [[HTTP](https://www.rfc-editor.org#HTTP)]) in the forwarded request to
...
response.[¶](https://www.rfc-editor.org#section-4.3-1)
...
identified by the Vary header field ([Section 4.1](https://www.rfc-editor.org#caching.negotiated.responses)).[¶](https://www.rfc-editor.org#section-4.3.1-1)
...
(see [Section 4.1](https://www.rfc-editor.org#caching.negotiated.responses)).[¶](https://www.rfc-editor.org#section-4.3.1-2)
...
representation of the resource.[¶](https://www.rfc-editor.org#section-4.3.1-3)

One such validator is the timestamp given in a Last-Modified
header field ([Section 8.8.2](https://www.rfc-editor.org/rfc/rfc9110#section-8.8.2) of [[HTTP](https://www.rfc-editor.org#HTTP)]), which can be used in an
...
specifically to a previously obtained representation with that timestamp).[¶](https://www.rfc-editor.org#section-4.3.1-4)

Another validator is the entity tag given in an ETag
field ([Section 8.8.3](https://www.rfc-editor.org/rfc/rfc9110#section-8.8.3) of [[HTTP](https://www.rfc-editor.org#HTTP)]). One or more entity tags, indicating one or more
...
representations with the listed entity tags).[¶](https://www.rfc-editor.org#section-4.3.1-5)

When generating a conditional request for validation, a cache:[¶](https://www.rfc-editor.org#section-4.3.1-6)

-
MUST send the relevant entity tags
(using If-Match, If-None-Match, or
If-Range) if the entity tags were provided in the
stored response(s) being validated.
[¶](https://www.rfc-editor.org#section-4.3.1-7.1) -
SHOULD send the Last-Modified value (using
...
[¶](https://www.rfc-editor.org#section-4.3.1-7.2) -
MAY send the Last-Modified value (using
...
(not an entity tag).
[¶](https://www.rfc-editor.org#section-4.3.1-7.3)
...
that do not understand entity tag preconditions to respond appropriately.[¶](https://www.rfc-editor.org#section-4.3.1-8)
...
complete the transfer of a partially retrieved representation.[¶](https://www.rfc-editor.org#section-4.3.2-1)

If a cache receives a request that can be satisfied by reusing a stored
200 (OK) or 206 (Partial Content)
response, as per [Section 4](https://www.rfc-editor.org#constructing.responses.from.caches),
...
corresponding validators contained within the stored response.[¶](https://www.rfc-editor.org#section-4.3.2-2)
...
intended for some other (inbound) server.[¶](https://www.rfc-editor.org#section-4.3.2-3)
...
If-Modified-Since. See [Section 13.2.2](https://www.rfc-editor.org/rfc/rfc9110#section-13.2.2) of [[HTTP](https://www.rfc-editor.org#HTTP)] for
a complete specification of precondition precedence.[¶](https://www.rfc-editor.org#section-4.3.2-4)

A request containing an If-None-Match header field ([Section 13.1.2](https://www.rfc-editor.org/rfc/rfc9110#section-13.1.2) of [[HTTP](https://www.rfc-editor.org#HTTP)]) indicates that the client wants to
validate one or more of its own stored responses in comparison to the
stored response chosen by the cache (as per [Section 4](https://www.rfc-editor.org#constructing.responses.from.caches)).[¶](https://www.rfc-editor.org#section-4.3.2-5)

If an If-None-Match header field is not present, a request
containing an If-Modified-Since header field
([Section 13.1.3](https://www.rfc-editor.org/rfc/rfc9110#section-13.1.3) of [[HTTP](https://www.rfc-editor.org#HTTP)]) indicates that the client wants to validate
one or more of its own stored responses by modification date.[¶](https://www.rfc-editor.org#section-4.3.2-6)
...
that the stored response was received) to evaluate the conditional.[¶](https://www.rfc-editor.org#section-4.3.2-7)

A cache that implements partial responses to range requests, as defined in
[Section 14.2](https://www.rfc-editor.org/rfc/rfc9110#section-14.2) of [[HTTP](https://www.rfc-editor.org#HTTP)], also needs to evaluate a received
If-Range header field ([Section 13.1.5](https://www.rfc-editor.org/rfc/rfc9110#section-13.1.5) of [[HTTP](https://www.rfc-editor.org#HTTP)])
with respect to the cache's chosen response.[¶](https://www.rfc-editor.org#section-4.3.2-8)
...
([Section 4.3.4](https://www.rfc-editor.org#freshening.responses)).[¶](https://www.rfc-editor.org#section-4.3.2-9)

Cache handling of a response to a conditional request depends upon its
status code:[¶](https://www.rfc-editor.org#section-4.3.3-1)

-
A 304 (Not Modified) response status code indicates
that the stored response can be updated and reused; see
[Section 4.3.4](https://www.rfc-editor.org#freshening.responses).[¶](https://www.rfc-editor.org#section-4.3.3-2.1) -
...
[Section 3](https://www.rfc-editor.org#response.cacheability)).[¶](https://www.rfc-editor.org#section-4.3.3-2.2) -
...
[Section 4.2.4](https://www.rfc-editor.org#serving.stale.responses)), or retry the validation request.[¶](https://www.rfc-editor.org#section-4.3.3-2.3)
...
provided, and then do so.[¶](https://www.rfc-editor.org#section-4.3.4-1)

The initial set of stored responses to update are those that could have been chosen for
that request -- i.e., those that meet the requirements in [Section 4](https://www.rfc-editor.org#constructing.responses.from.caches), except the last requirement
to be fresh, able to be served stale, or just validated.[¶](https://www.rfc-editor.org#section-4.3.4-2)

Then, that initial set of stored responses is further filtered by the first match of:[¶](https://www.rfc-editor.org#section-4.3.4-3)

-
If the new response contains one or more "strong validators" (see
[Section 8.8.1](https://www.rfc-editor.org/rfc/rfc9110#section-8.8.1)of [[HTTP](https://www.rfc-editor.org#HTTP)]), then each of those strong validators identifies a selected representation for update. All the stored responses in the initial set with one of those same strong validators are identified for update. If none of the initial set contains at least one of the same strong validators, then the cache MUST NOT use the new response to update any stored responses.[¶](https://www.rfc-editor.org#section-4.3.4-4.1) -
...
[¶](https://www.rfc-editor.org#section-4.3.4-4.2) -
...
[¶](https://www.rfc-editor.org#section-4.3.4-4.3)
...
Modified) response, as per [Section 3.2](https://www.rfc-editor.org#update).[¶](https://www.rfc-editor.org#section-4.3.4-5)
...
changed.[¶](https://www.rfc-editor.org#section-4.3.5-1)
...
for that request (see [Section 4.1](https://www.rfc-editor.org#caching.negotiated.responses)).[¶](https://www.rfc-editor.org#section-4.3.5-2)
...
otherwise, the cache SHOULD consider the stored response to be stale.[¶](https://www.rfc-editor.org#section-4.3.5-3)
...
response to update the stored response (see [Section 3.2](https://www.rfc-editor.org#update)).[¶](https://www.rfc-editor.org#section-4.3.5-4)

Because unsafe request methods ([Section 9.2.1](https://www.rfc-editor.org/rfc/rfc9110#section-9.2.1) of [[HTTP](https://www.rfc-editor.org#HTTP)]) such as PUT, POST, or DELETE
have the potential for changing state on the origin server, intervening
caches are required to invalidate stored responses to keep their contents up to date.[¶](https://www.rfc-editor.org#section-4.4-1)

A cache MUST invalidate the target URI
([Section 7.1](https://www.rfc-editor.org/rfc/rfc9110#section-7.1) of [[HTTP](https://www.rfc-editor.org#HTTP)]) when it receives a non-error status
code in response to
an unsafe request method (including methods whose safety is unknown).[¶](https://www.rfc-editor.org#section-4.4-2)
...
if the origin ([Section 4.3.1](https://www.rfc-editor.org/rfc/rfc9110#section-4.3.1) of [[HTTP](https://www.rfc-editor.org#HTTP)]) of the URI to be invalidated differs from that of the target URI
([Section 7.1](https://www.rfc-editor.org/rfc/rfc9110#section-7.1) of [[HTTP](https://www.rfc-editor.org#HTTP)]). This helps prevent denial-of-service attacks.[¶](https://www.rfc-editor.org#section-4.4-3)
...
in response to a subsequent request.[¶](https://www.rfc-editor.org#section-4.4-4)

A "non-error response" is one with a 2xx (Successful)
or 3xx (Redirection) status code.[¶](https://www.rfc-editor.org#section-4.4-5)

Note that this does not guarantee that all appropriate responses are
invalidated globally; a state-changing request would only invalidate
responses in the caches it travels through.[¶](https://www.rfc-editor.org#section-4.4-6)

This section defines the syntax and semantics of HTTP fields
related to caching.[¶](https://www.rfc-editor.org#section-5-1)

The "Age" response header field conveys the sender's estimate of the
time since the response was generated or successfully validated at the
origin server. Age values are calculated as specified in [Section 4.2.3](https://www.rfc-editor.org#age.calculations).[¶](https://www.rfc-editor.org#section-5.1-1)

Age = delta-seconds

The Age field value is a non-negative integer, representing time in seconds
(see [Section 1.2.2](https://www.rfc-editor.org#delta-seconds)).[¶](https://www.rfc-editor.org#section-5.1-3)
...
first member of the field value, discarding subsequent ones.[¶](https://www.rfc-editor.org#section-5.1-4)
...
a cache SHOULD ignore the field.[¶](https://www.rfc-editor.org#section-5.1-5)

The presence of an Age header field implies that the response was not
generated or validated by the origin server for this request. However,
lack of an Age header field does not imply the origin was contacted.[¶](https://www.rfc-editor.org#section-5.1-6)
...
same directive is present or copied in the response.[¶](https://www.rfc-editor.org#section-5.2-1)

See [Section 5.2.3](https://www.rfc-editor.org#cache.control.extensions) for information about how
Cache-Control directives defined elsewhere are handled.[¶](https://www.rfc-editor.org#section-5.2-2)
...
target a directive to a specific cache.[¶](https://www.rfc-editor.org#section-5.2-3)
...
ought to accept both forms, even if a specific form is required for generation.[¶](https://www.rfc-editor.org#section-5.2-4)
...
unless stated otherwise.[¶](https://www.rfc-editor.org#section-5.2-6)

This section defines cache request directives. They are advisory; caches
MAY implement them, but are not required to.[¶](https://www.rfc-editor.org#section-5.2.1-1)

Argument syntax:[¶](https://www.rfc-editor.org#section-5.2.1.1-1)

-
[delta-seconds](https://www.rfc-editor.org#delta-seconds)(see[Section 1.2.2](https://www.rfc-editor.org#delta-seconds))[¶](https://www.rfc-editor.org#section-5.2.1.1-2.1)
...
client does not wish to receive a stale response.[¶](https://www.rfc-editor.org#section-5.2.1.1-3)

This directive uses the token form of the argument syntax:
e.g., 'max-age=5' not 'max-age="5"'. A sender MUST NOT generate the
quoted-string form.[¶](https://www.rfc-editor.org#section-5.2.1.1-4)

Argument syntax:[¶](https://www.rfc-editor.org#section-5.2.1.2-1)

-
[delta-seconds](https://www.rfc-editor.org#delta-seconds)(see[Section 1.2.2](https://www.rfc-editor.org#delta-seconds))[¶](https://www.rfc-editor.org#section-5.2.1.2-2.1)
...
stale response of any age.[¶](https://www.rfc-editor.org#section-5.2.1.2-3)

This directive uses the token form of the argument syntax:
e.g., 'max-stale=10' not 'max-stale="10"'. A sender MUST NOT generate
the quoted-string form.[¶](https://www.rfc-editor.org#section-5.2.1.2-4)

Argument syntax:[¶](https://www.rfc-editor.org#section-5.2.1.3-1)

-
[delta-seconds](https://www.rfc-editor.org#delta-seconds)(see[Section 1.2.2](https://www.rfc-editor.org#delta-seconds))[¶](https://www.rfc-editor.org#section-5.2.1.3-2.1)
...
still be fresh for at least the specified number of seconds.[¶](https://www.rfc-editor.org#section-5.2.1.3-3)

This directive uses the token form of the argument syntax:
e.g., 'min-fresh=20' not 'min-fresh="20"'. A sender MUST NOT generate
the quoted-string form.[¶](https://www.rfc-editor.org#section-5.2.1.3-4)
...
validation on the origin server.[¶](https://www.rfc-editor.org#section-5.2.1.4-1)
...
promptly as possible after forwarding it.[¶](https://www.rfc-editor.org#section-5.2.1.5-1)

This directive is not a reliable or sufficient mechanism for ensuring
privacy. In particular, malicious or compromised caches might not
recognize or obey this directive, and communications networks might be
vulnerable to eavesdropping.[¶](https://www.rfc-editor.org#section-5.2.1.5-2)

Note that if a request containing this directive is satisfied from a
cache, the no-store request directive does not apply to the already
stored response.[¶](https://www.rfc-editor.org#section-5.2.1.5-3)

The no-transform request directive indicates that the client is asking
for intermediaries to avoid
transforming the content, as defined in [Section 7.7](https://www.rfc-editor.org/rfc/rfc9110#section-7.7) of [[HTTP](https://www.rfc-editor.org#HTTP)].[¶](https://www.rfc-editor.org#section-5.2.1.6-1)
...
a 504 (Gateway Timeout) status code.[¶](https://www.rfc-editor.org#section-5.2.1.7-1)

This section defines cache response directives. A cache MUST obey the
Cache-Control directives defined in this section.[¶](https://www.rfc-editor.org#section-5.2.2-1)

Argument syntax:[¶](https://www.rfc-editor.org#section-5.2.2.1-1)

-
[delta-seconds](https://www.rfc-editor.org#delta-seconds)(see[Section 1.2.2](https://www.rfc-editor.org#delta-seconds))[¶](https://www.rfc-editor.org#section-5.2.2.1-2.1)

The max-age response directive indicates that the response is to be
considered stale after its age is greater than the specified number of
seconds.[¶](https://www.rfc-editor.org#section-5.2.2.1-3)

This directive uses the token form of the argument syntax:
e.g., 'max-age=5' not 'max-age="5"'. A sender MUST NOT generate the
quoted-string form.[¶](https://www.rfc-editor.org#section-5.2.2.1-4)
...
defined by [Section 4.3](https://www.rfc-editor.org#validation.model).[¶](https://www.rfc-editor.org#section-5.2.2.2-1)
...
The generated status code SHOULD be 504 (Gateway Timeout)
unless another error status code is more applicable.[¶](https://www.rfc-editor.org#section-5.2.2.2-2)

The must-revalidate directive ought to be used by servers if and only
if failure to validate a request could cause
incorrect operation, such as a silently unexecuted financial
transaction.[¶](https://www.rfc-editor.org#section-5.2.2.2-3)

The must-revalidate directive also permits a shared cache to
reuse a response to a request containing an Authorization
header field ([Section 11.6.2](https://www.rfc-editor.org/rfc/rfc9110#section-11.6.2) of [[HTTP](https://www.rfc-editor.org#HTTP)]),
subject to the above requirement on revalidation
([Section 3.5](https://www.rfc-editor.org#caching.authenticated.responses)).[¶](https://www.rfc-editor.org#section-5.2.2.2-4)
...
response's status code.[¶](https://www.rfc-editor.org#section-5.2.2.3-1)
...
understands and implements the status code's caching requirements.[¶](https://www.rfc-editor.org#section-5.2.2.3-2)

Argument syntax:[¶](https://www.rfc-editor.org#section-5.2.2.4-1)
...
successful response; see [Section 4.3](https://www.rfc-editor.org#validation.model).[¶](https://www.rfc-editor.org#section-5.2.2.4-3)
...
been configured to send stale responses.[¶](https://www.rfc-editor.org#section-5.2.2.4-4)
...
response.[¶](https://www.rfc-editor.org#section-5.2.2.4-5)

The field names given are not limited to the set of header
fields defined by this specification. Field names are case-insensitive.[¶](https://www.rfc-editor.org#section-5.2.2.4-6)
...
to be needed for single-entry lists).[¶](https://www.rfc-editor.org#section-5.2.2.4-7)
...
the response to satisfy any other request.[¶](https://www.rfc-editor.org#section-5.2.2.5-1)
...
possible after forwarding it.[¶](https://www.rfc-editor.org#section-5.2.2.5-2)

This directive is not a reliable or sufficient mechanism for ensuring
privacy. In particular, malicious or compromised caches might not
recognize or obey this directive, and communications networks might be
vulnerable to eavesdropping.[¶](https://www.rfc-editor.org#section-5.2.2.5-3)

Note that the must-understand cache directive overrides no-store in certain
circumstances; see [Section 5.2.2.3](https://www.rfc-editor.org#cache-response-directive.must-understand).[¶](https://www.rfc-editor.org#section-5.2.2.5-4)

The no-transform response directive indicates that an intermediary
(regardless of whether it implements a cache) MUST NOT transform the
content, as defined in [Section 7.7](https://www.rfc-editor.org/rfc/rfc9110#section-7.7) of [[HTTP](https://www.rfc-editor.org#HTTP)].[¶](https://www.rfc-editor.org#section-5.2.2.6-1)

Argument syntax:[¶](https://www.rfc-editor.org#section-5.2.2.7-1)
...
to the constraints defined in [Section 3](https://www.rfc-editor.org#response.cacheability), even if
the response would not otherwise be heuristically cacheable by a private
cache.[¶](https://www.rfc-editor.org#section-5.2.2.7-3)
...
the constraints defined in [Section 3](https://www.rfc-editor.org#response.cacheability).[¶](https://www.rfc-editor.org#section-5.2.2.7-4)

The field names given are not limited to the set of header
fields defined by this specification. Field names are case-insensitive.[¶](https://www.rfc-editor.org#section-5.2.2.7-5)

This directive uses the quoted-string form of the argument syntax.
A sender SHOULD NOT generate the token form (even if quoting appears not
to be needed for single-entry lists).[¶](https://www.rfc-editor.org#section-5.2.2.7-6)
...
as defined by [Section 4.3](https://www.rfc-editor.org#validation.model). This is analogous to
must-revalidate ([Section 5.2.2.2](https://www.rfc-editor.org#cache-response-directive.must-revalidate)),
except that proxy-revalidate does not apply to private caches.[¶](https://www.rfc-editor.org#section-5.2.2.8-1)
...
([Section 5.2.2.9](https://www.rfc-editor.org#cache-response-directive.public)), allowing the response
to be cached while requiring only a shared cache to revalidate when stale.[¶](https://www.rfc-editor.org#section-5.2.2.8-2)
...
constraints defined in [Section 3](https://www.rfc-editor.org#response.cacheability). In other words,
...
an Authorization header field ([Section 3.5](https://www.rfc-editor.org#caching.authenticated.responses)).[¶](https://www.rfc-editor.org#section-5.2.2.9-1)

Note that it is unnecessary to add the public directive to a response that
is already cacheable according to [Section 3](https://www.rfc-editor.org#response.cacheability).[¶](https://www.rfc-editor.org#section-5.2.2.9-2)

If a response with the public directive has no explicit freshness information,
it is heuristically cacheable ([Section 4.2.2](https://www.rfc-editor.org#heuristic.freshness)).[¶](https://www.rfc-editor.org#section-5.2.2.9-3)

Argument syntax:[¶](https://www.rfc-editor.org#section-5.2.2.10-1)

-
[delta-seconds](https://www.rfc-editor.org#delta-seconds)(see[Section 1.2.2](https://www.rfc-editor.org#delta-seconds))[¶](https://www.rfc-editor.org#section-5.2.2.10-2.1)
...
specified by either the max-age directive or the [Expires](https://www.rfc-editor.org#field.expires)
header field.[¶](https://www.rfc-editor.org#section-5.2.2.10-3)

The s-maxage directive incorporates the
semantics of the proxy‑revalidate response directive ([Section 5.2.2.8](https://www.rfc-editor.org#cache-response-directive.proxy-revalidate))
for a shared cache.
A shared cache MUST NOT reuse a stale response with s-maxage to satisfy
another request until it has been successfully validated by the origin, as
defined by [Section 4.3](https://www.rfc-editor.org#validation.model).
...
([Section 3.5](https://www.rfc-editor.org#caching.authenticated.responses)).[¶](https://www.rfc-editor.org#section-5.2.2.10-4)

This directive uses the token form of the argument syntax:
e.g., 's-maxage=10' not 's-maxage="10"'. A sender MUST NOT generate
the quoted-string form.[¶](https://www.rfc-editor.org#section-5.2.2.10-5)

The Cache-Control header field can be extended through the use of one or
more extension cache directives.
A cache MUST ignore unrecognized cache directives.[¶](https://www.rfc-editor.org#section-5.2.3-1)

Informational extensions (those that do not require a change in cache
behavior) can be added without changing the semantics of other directives.[¶](https://www.rfc-editor.org#section-5.2.3-2)
...
cache directives can be made without breaking deployed caches.[¶](https://www.rfc-editor.org#section-5.2.3-3)
...
shared cache(s) could do so by including[¶](https://www.rfc-editor.org#section-5.2.3-4)
...
private directive.[¶](https://www.rfc-editor.org#section-5.2.3-6)

New extension directives ought to consider defining:[¶](https://www.rfc-editor.org#section-5.2.3-7)

- What it means for a directive to be specified multiple times,
[¶](https://www.rfc-editor.org#section-5.2.3-8.1) - When the directive does not take an argument, what it means when an
argument is present,
[¶](https://www.rfc-editor.org#section-5.2.3-8.2) - When the directive requires an argument, what it means when it is
missing, and
[¶](https://www.rfc-editor.org#section-5.2.3-8.3) - Whether the directive is specific to requests, specific to responses, or able
to be used in either.
[¶](https://www.rfc-editor.org#section-5.2.3-8.4)
...
<[https://www.iana.org/assignments/http-cache-directives](https://www.iana.org/assignments/http-cache-directives)>.[¶](https://www.rfc-editor.org#section-5.2.4-1)

A registration MUST include the following fields:[¶](https://www.rfc-editor.org#section-5.2.4-2)

Values to be added to this namespace require IETF Review (see [[RFC8126](https://www.rfc-editor.org#RFC8126)], [Section 4.8](https://www.rfc-editor.org/rfc/rfc8126#section-4.8)).[¶](https://www.rfc-editor.org#section-5.2.4-4)

The "Expires" response header field gives the date/time after which the
response is considered stale. See [Section 4.2](https://www.rfc-editor.org#expiration.model) for
further discussion of the freshness model.[¶](https://www.rfc-editor.org#section-5.3-1)

The presence of an Expires header field does not imply that the original resource
will change or cease to exist at, before, or after that time.[¶](https://www.rfc-editor.org#section-5.3-2)

The Expires field value is an HTTP-date timestamp, as defined in [Section 5.6.7](https://www.rfc-editor.org/rfc/rfc9110#section-5.6.7) of [[HTTP](https://www.rfc-editor.org#HTTP)].
See also [Section 4.2](https://www.rfc-editor.org#expiration.model) for parsing requirements specific to caches.[¶](https://www.rfc-editor.org#section-5.3-3)

Expires = HTTP-date

For example[¶](https://www.rfc-editor.org#section-5.3-5)

A cache recipient MUST interpret invalid date formats, especially the
value "0", as representing a time in the past (i.e., "already expired").[¶](https://www.rfc-editor.org#section-5.3-7)

If a response includes a [Cache-Control](https://www.rfc-editor.org#field.cache-control) header field with
the max-age directive ([Section 5.2.2.1](https://www.rfc-editor.org#cache-response-directive.max-age)),
a recipient MUST ignore the Expires header field.
Likewise, if a response includes the s-maxage directive
([Section 5.2.2.10](https://www.rfc-editor.org#cache-response-directive.s-maxage)), a shared cache
...
the Cache-Control header field.[¶](https://www.rfc-editor.org#section-5.3-8)

An origin server without a clock ([Section 5.6.7](https://www.rfc-editor.org/rfc/rfc9110#section-5.6.7) of [[HTTP](https://www.rfc-editor.org#HTTP)])
...
a clock.[¶](https://www.rfc-editor.org#section-5.3-9)
...
that.[¶](https://www.rfc-editor.org#section-5.3-10)

The "Pragma" request header field was defined for HTTP/1.0 caches, so that clients
could specify a "no-cache" request (as [Cache-Control](https://www.rfc-editor.org#field.cache-control) was
not defined until HTTP/1.1).[¶](https://www.rfc-editor.org#section-5.4-1)

However, support for Cache-Control is now widespread. As a result, this
specification deprecates Pragma.[¶](https://www.rfc-editor.org#section-5.4-2)
...
from examining other header fields, such as [Age](https://www.rfc-editor.org#field.age).[¶](https://www.rfc-editor.org#section-5.5-1)
...
session.[¶](https://www.rfc-editor.org#section-6-1)

Likewise, some Web browsers implement caching of images and other assets
within a page view; they may or may not honor HTTP caching semantics.[¶](https://www.rfc-editor.org#section-6-2)
...
freshness lifetime.[¶](https://www.rfc-editor.org#section-6-3)
...
no-store).[¶](https://www.rfc-editor.org#section-6-4)
...
which it was obtained.[¶](https://www.rfc-editor.org#section-6-5)
...
([Section 11](https://www.rfc-editor.org/rfc/rfc9112#section-11) of [[HTTP/1.1](https://www.rfc-editor.org#HTTP11)])
and "HTTP Semantics"
([Section 17](https://www.rfc-editor.org/rfc/rfc9110#section-17) of [[HTTP](https://www.rfc-editor.org#HTTP)]).[¶](https://www.rfc-editor.org#section-7-1)
...
need to be protected as sensitive information.[¶](https://www.rfc-editor.org#section-7-2)
...
by allowing stored responses to be removed for some or all origin servers.[¶](https://www.rfc-editor.org#section-7-3)
...
are used to distribute malicious content to many clients.[¶](https://www.rfc-editor.org#section-7.1-1)

One common attack vector for cache poisoning is to exploit differences in
message parsing on proxies and in user agents; see [Section 6.3](https://www.rfc-editor.org/rfc/rfc9112#section-6.3) of [[HTTP/1.1](https://www.rfc-editor.org#HTTP11)] for the relevant requirements regarding
HTTP/1.1.[¶](https://www.rfc-editor.org#section-7.1-2)
...
requested.[¶](https://www.rfc-editor.org#section-7.2-1)
...
a specific page on it.[¶](https://www.rfc-editor.org#section-7.2-2)
...
attack described above). This is sometimes called "double keying".[¶](https://www.rfc-editor.org#section-7.2-3)
...
unauthorized parties.[¶](https://www.rfc-editor.org#section-7.3-1)

Note that the Set-Cookie response header field [[COOKIE](https://www.rfc-editor.org#COOKIE)]
...
emit appropriate Cache-Control response header fields.[¶](https://www.rfc-editor.org#section-7.3-2)

The change controller for the following registrations is:
"IETF (iesg@ietf.org) - Internet Engineering Task Force".[¶](https://www.rfc-editor.org#section-8-1)

IANA has updated the "Hypertext Transfer Protocol (HTTP) Field
Name Registry" at <[https://www.iana.org/assignments/http-fields](https://www.iana.org/assignments/http-fields)>,
as described in [Section 18.4](https://www.rfc-editor.org/rfc/rfc9110#section-18.4) of [[HTTP](https://www.rfc-editor.org#HTTP)],
with the field names listed in the table below:[¶](https://www.rfc-editor.org#section-8.1-1)

IANA has updated the
"Hypertext Transfer Protocol (HTTP) Cache Directive Registry"
at <[https://www.iana.org/assignments/http-cache-directives](https://www.iana.org/assignments/http-cache-directives)>
with the registration procedure per [Section 5.2.4](https://www.rfc-editor.org#cache.directive.registry)
and the cache directive names summarized in the table below.[¶](https://www.rfc-editor.org#section-8.2-1)
...
registry at <[https://www.iana.org/assignments/http-warn-codes](https://www.iana.org/assignments/http-warn-codes)>
stating that "Warning" has been obsoleted:[¶](https://www.rfc-editor.org#section-8.3-1)
...
[https://www.rfc-editor.org/info/rfc2119](https://www.rfc-editor.org/info/rfc2119)>. - [RFC5234]
-
Crocker, D., Ed. and P. Overell, "Augmented BNF for Syntax Specifications: ABNF", STD 68, RFC 5234, DOI 10.17487/RFC5234, , <
[https://www.rfc-editor.org/info/rfc5234](https://www.rfc-editor.org/info/rfc5234)>. - [RFC7405]
-
Kyzivat, P., "Case-Sensitive String Support in ABNF", RFC 7405, DOI 10.17487/RFC7405, , <
[https://www.rfc-editor.org/info/rfc7405](https://www.rfc-editor.org/info/rfc7405)>. - [RFC8174]
-
...
[https://www.rfc-editor.org/info/rfc6265](https://www.rfc-editor.org/info/rfc6265)>. - [HTTP/1.1]
-
Fielding, R., Ed., Nottingham, M., Ed., and J. Reschke, Ed., "HTTP/1.1", STD 99, RFC 9112, DOI 10.17487/RFC9112, , <
[https://www.rfc-editor.org/info/rfc9112](https://www.rfc-editor.org/info/rfc9112)>. - [RFC2616]
-
...
[https://www.rfc-editor.org/info/rfc2616](https://www.rfc-editor.org/info/rfc2616)>. - [RFC5861]
-
Nottingham, M., "HTTP Cache-Control Extensions for Stale Content", RFC 5861, DOI 10.17487/RFC5861, , <
[https://www.rfc-editor.org/info/rfc5861](https://www.rfc-editor.org/info/rfc5861)>. - [RFC7234]
-
...
[https://www.rfc-editor.org/info/rfc7234](https://www.rfc-editor.org/info/rfc7234)>. - [RFC8126]
-
...
In the collected ABNF below, list rules are expanded per [Section 5.6.1](https://www.rfc-editor.org/rfc/rfc9110#section-5.6.1) of [[HTTP](https://www.rfc-editor.org#HTTP)].[¶](https://www.rfc-editor.org#appendix-A-1)
...
([Section 4.2.1](https://www.rfc-editor.org#calculating.freshness.lifetime))[¶](https://www.rfc-editor.org#appendix-B-1)
...
([Section 4.4](https://www.rfc-editor.org#invalidation))[¶](https://www.rfc-editor.org#appendix-B-2)
...
([Section 4.4](https://www.rfc-editor.org#invalidation))[¶](https://www.rfc-editor.org#appendix-B-3)

Handling invalid and multiple Age header field values has been clarified.
([Section 5.1](https://www.rfc-editor.org#field.age))[¶](https://www.rfc-editor.org#appendix-B-4)
...
unknown extensions.
([Section 5.2](https://www.rfc-editor.org#field.cache-control))[¶](https://www.rfc-editor.org#appendix-B-5)

The public and private cache directives were clarified, so that they
do not make responses reusable under any condition.
([Section 5.2.2](https://www.rfc-editor.org#cache-response-directive))[¶](https://www.rfc-editor.org#appendix-B-6)
...
([Section 5.2.2.3](https://www.rfc-editor.org#cache-response-directive.must-understand))[¶](https://www.rfc-editor.org#appendix-B-7)
...
([Section 5.5](https://www.rfc-editor.org#field.warning))[¶](https://www.rfc-editor.org#appendix-B-8)

See Appendix "Acknowledgements" of [[HTTP](https://www.rfc-editor.org#HTTP)], which applies to this document as well.[¶](https://www.rfc-editor.org#appendix-C-1)
