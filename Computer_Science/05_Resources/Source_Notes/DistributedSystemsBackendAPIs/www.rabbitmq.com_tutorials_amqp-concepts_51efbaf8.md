...
Source URL: https://www.rabbitmq.com/tutorials/amqp-concepts
Source Type: official_docs
Trust Score: 93
Fetched At: 2026-04-17T07:09:18+00:00
...
## Overview[](https://www.rabbitmq.com#overview)

This guide provides an overview of the AMQP 0-9-1 protocol, one of the protocols supported by RabbitMQ.

## High-level Overview of AMQP 0-9-1 and the AMQP Model[](https://www.rabbitmq.com#highlevel-overview)

### What is AMQP 0-9-1?[](https://www.rabbitmq.com#what-is-amqp)
...
### Brokers and Their Role[](https://www.rabbitmq.com#brokers-role)
...
[consumers](https://www.rabbitmq.com/docs/consumers)Since it is a network protocol, the publishers, consumers and the broker can all reside on different machines.

### AMQP 0-9-1 Model in Brief[](https://www.rabbitmq.com#amqp-model)
...
*bindings*. Then the broker either delivers messages to
...
*message attributes* (message meta-data). Some of this
...
*message acknowledgements*: when a message is delivered
to a consumer the consumer *notifies the broker*,
...
be routed, messages may be *returned* to publishers,
...
### AMQP 0-9-1 is a Programmable Protocol[](https://www.rabbitmq.com#programmable-protocol)
...
subscribe to [queues](https://www.rabbitmq.com/docs/queues) and so on.
...
## Exchanges and Exchange Types[](https://www.rabbitmq.com#exchanges)

*Exchanges* are AMQP 0-9-1 entities where messages are
sent to. Exchanges take a message and route it into zero or
more queues. The routing algorithm used depends on the
*exchange type* and rules called *bindings*.
...
### Default Exchange[](https://www.rabbitmq.com#exchange-default)
...
### Direct Exchange[](https://www.rabbitmq.com#exchange-direct)
...
### Fanout Exchange[](https://www.rabbitmq.com#exchange-fanout)
...
### Topic Exchange[](https://www.rabbitmq.com#exchange-topic)
...
### Headers Exchange[](https://www.rabbitmq.com#exchange-headers)
...
[Queues](https://www.rabbitmq.com/docs/queues) in the AMQP 0-9-1 model are very similar to queues in
...
### Queue Names[](https://www.rabbitmq.com#queue-names)
...
### Queue Durability[](https://www.rabbitmq.com#queue-durability)
...
## Bindings[](https://www.rabbitmq.com#bindings)
...
Q has to be *bound* to E. Bindings may have an optional
*routing key* attribute used by some exchange types. The
...
to) it is either [dropped or returned to the publisher](https://www.rabbitmq.com/docs/publishers#unroutable),
...
can * consume* them. In the AMQP 0-9-1 Model, there
...
- Polling ("pull API"): this way is
**highly inefficient**and**should be avoided**in most cases

With the "push API", applications have to indicate interest in
consuming messages from a particular queue. When they do so,
we say that they *register a consumer*
or, simply put, *subscribe to a queue*. It is possible
to have more than one consumer per queue or to register an
*exclusive consumer* (excludes all other consumers from
...
### Message Acknowledgements[](https://www.rabbitmq.com#consumer-acknowledgements)

[Consumer applications](https://www.rabbitmq.com/docs/consumers) – that is, applications that receive and process
...
two [acknowledgement modes](https://www.rabbitmq.com/docs/confirms):
...
### Rejecting Messages[](https://www.rabbitmq.com#messages-reject)
...
### Negative Acknowledgements[](https://www.rabbitmq.com#messages-nack)

Messages are rejected with the `basic.reject`

method.
There is one limitation that `basic.reject`
...
more information, please refer to the [Confirmations](https://www.rabbitmq.com/docs/confirms)
and [basic.nack extension](https://www.rabbitmq.com/docs/nack) guides.

### Prefetching Messages[](https://www.rabbitmq.com#messages-prefetch)
...
## Message Attributes and Payload[](https://www.rabbitmq.com#messages)

Messages in the AMQP 0-9-1 model have *attributes*. Some
...
as *headers*. They are similar to X-Headers in
HTTP. Message attributes are set when a message is published.

Messages also have a *payload* (the data that
...
## AMQP 0-9-1 Methods[](https://www.rabbitmq.com#amqp-methods)

AMQP 0-9-1 is structured as a number of
*methods*. Methods are operations (like HTTP methods)
...
*classes*. Classes are just logical groupings of AMQP 0-9-1
...
Let us take a look at the *exchange* class, a group
...
exchange using the `exchange.declare`
...
If the operation succeeds, the broker responds with the `exchange.declare-ok`
...
method pair on the AMQP 0-9-1 *queue* method class: `queue.declare`
...
and some others (`basic.get`

, for example)
have more than one possible "response".

## Connections[](https://www.rabbitmq.com#amqp-connections)
...
## Channels[](https://www.rabbitmq.com#amqp-channels)
...
## Virtual Hosts[](https://www.rabbitmq.com#amqp-vhosts)

To make it possible for a single broker to host multiple
isolated "environments" (groups of users, exchanges,
queues and so on), AMQP 0-9-1 includes the concept of * virtual hosts* (vhosts).
...
## AMQP is Extensible[](https://www.rabbitmq.com#amqp-extensibility)

AMQP 0-9-1 has several extension points:

[Custom exchange types](https://www.rabbitmq.com/client-libraries/devtools#miscellaneous)let developers implement routing schemes that exchange types provided out-of-the-box do not cover well, for example, geodata-based routing.- Declaration of exchanges and queues can include additional attributes that the broker
can use. For example,
[per-queue message TTL](https://www.rabbitmq.com/docs/ttl)in RabbitMQ is implemented this way. - Broker-specific extensions to the protocol. See, for example,
[extensions that RabbitMQ implements](https://www.rabbitmq.com/docs/extensions). - Every now and then, new AMQP 0-9-1 method classes can be introduced for specific cases such
as
[OAuth 2](https://www.rabbitmq.com/docs/oauth2)(JWT) token refresh. - Brokers can be extended with
[additional plugins](https://www.rabbitmq.com/docs/plugins), for example, the[RabbitMQ management](https://www.rabbitmq.com/docs/management)frontend and HTTP API are implemented as a plugin.

These features make the AMQP 0-9-1 Model even more flexible and applicable to a very broad range of problems.

## AMQP 0-9-1 Clients Ecosystem[](https://www.rabbitmq.com#amqp-clients)

There are [many AMQP 0-9-1 clients](https://www.rabbitmq.com/client-libraries/devtools) for many
