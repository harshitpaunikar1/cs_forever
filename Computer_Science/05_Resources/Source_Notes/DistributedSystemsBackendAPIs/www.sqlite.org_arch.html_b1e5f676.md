...
Source URL: https://www.sqlite.org/arch.html
Source Type: official_docs
Trust Score: 96
Fetched At: 2026-04-17T07:08:17+00:00
...
SQLite works by compiling SQL text into [bytecode](https://www.sqlite.org/opcode.html), then running
that bytecode using a virtual machine.

The [sqlite3_prepare_v2()](https://www.sqlite.org/c3ref/prepare.html) and related interfaces act as a compiler
for converting SQL text into bytecode. The [sqlite3_stmt](https://www.sqlite.org/c3ref/stmt.html) object is
a container for a single bytecode program that implements a single
SQL statement. The [sqlite3_step()](https://www.sqlite.org/c3ref/step.html) interface passes a bytecode program
...
[interrupted](https://www.sqlite.org/c3ref/interrupt.html).

Much of the [C-language Interface](https://www.sqlite.org/c3ref/intro.html) is found in source
...
The [sqlite3_get_table()](https://www.sqlite.org/c3ref/free_table.html) routine is implemented in [table.c](https://sqlite.org/src/file/src/table.c).
The [sqlite3_mprintf()](https://www.sqlite.org/c3ref/mprintf.html) routine is found in [printf.c](https://sqlite.org/src/file/src/printf.c).
The [sqlite3_complete()](https://www.sqlite.org/c3ref/complete.html) interface is in [complete.c](https://sqlite.org/src/file/src/complete.c).
The [TCL Interface](https://www.sqlite.org/tclsqlite.html) is implemented by [tclsqlite.c](https://sqlite.org/src/file/src/tclsqlite.c).
...
The code generator, and especially the logic in The The virtual machine itself is entirely contained in a single
source file
...
the built-in SQL functions (ex: An SQLite database is maintained on disk using a B-tree implementation
found in the The interface to the B-tree subsystem and the rest of the SQLite library
is defined by the header file The B-tree module requests information from the disk in fixed-size
pages. The default The primary page cache implementation is in the
In order to provide portability across operating systems,
SQLite uses an abstract object called the
...
[bytecode](https://www.sqlite.org/opcode.html) that performs the work of the SQL statement.
The [prepared statement](https://www.sqlite.org/c3ref/stmt.html) object is a container for this bytecode.
...
[sqlite3_set_authorizer()](https://www.sqlite.org/c3ref/set_authorizer.html).**where*.c**
and in [select.c](https://sqlite.org/src/file/src/select.c), is sometimes called the
[query planner](https://www.sqlite.org/optoverview.html). For any particular SQL statement, there might be
...
[bytecode](https://www.sqlite.org/opcode.html) program created by the code generator is run by
a virtual machine.
[vdbe.c](https://sqlite.org/src/file/src/vdbe.c). The
...
[sqlite3_bind_int()](https://www.sqlite.org/c3ref/bind_blob.html) and [sqlite3_step()](https://www.sqlite.org/c3ref/step.html). Individual values
(strings, integer, floating point numbers, and BLOBs) are stored
in an internal object named "Mem" which is implemented by
[vdbemem.c](https://sqlite.org/src/file/src/vdbemem.c).[abs()](https://www.sqlite.org/lang_corefunc.html#abs), [count()](https://www.sqlite.org/lang_aggfunc.html#count),
[substr()](https://www.sqlite.org/lang_corefunc.html#substr), and so forth) can be found in the [func.c](https://sqlite.org/src/file/src/func.c) source
file.
Date and time conversion functions are found in [date.c](https://sqlite.org/src/file/src/date.c).
Some functions such as [coalesce()](https://www.sqlite.org/lang_corefunc.html#coalesce) and [typeof()](https://www.sqlite.org/lang_corefunc.html#typeof) are implemented
as bytecode directly by the code generator.
## B-Tree

[btree.c](https://sqlite.org/src/file/src/btree.c) source file. Separate B-trees are used for
each table and each index in the database. All B-trees are stored in the
same disk file. The [file format](https://www.sqlite.org/fileformat2.html) details are stable and well-defined and
are guaranteed to be compatible moving forward.[btree.h](https://sqlite.org/src/file/src/btree.h).
## Page Cache

[page_size](https://www.sqlite.org/pragma.html#pragma_page_size) is 4096 bytes but can be any power of
...
the requests are handled quickly, safely, and efficiently.[pager.c](https://sqlite.org/src/file/src/pager.c) file. [WAL mode](https://www.sqlite.org/wal.html) logic is in the separate
[wal.c](https://sqlite.org/src/file/src/wal.c) file. In-memory caching is implemented by the
...
[VFS](https://www.sqlite.org/vfs.html). Each VFS provides methods
...
SQLite currently provides VFSes for unix (in the [os_unix.c](https://sqlite.org/src/file/src/os_unix.c)
file) and Windows (in the [os_win.c](https://sqlite.org/src/file/src/os_win.c) file).
## Utilities

[util.c](https://sqlite.org/src/file/src/util.c).
...
[printf()](https://www.sqlite.org/printf.html) (with
