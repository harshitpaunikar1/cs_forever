...
Source URL: https://ffmpeg.org/developer.html
Source Type: official_docs
Trust Score: 96
Fetched At: 2026-04-17T07:09:46+00:00
...
[1 Introduction](https://ffmpeg.org#toc-Introduction)
...
-
[the Doxygen documentation](http://ffmpeg.org/doxygen/trunk/index.html)generated from the headers - the examples under
`doc/examples`
...
described in more detail in the [Submitting patches](https://ffmpeg.org#Submitting-patches) chapter. The code
should comply with the [Development Policy](https://ffmpeg.org#Development-Policy) and follow the [Coding Rules](https://ffmpeg.org#Coding-Rules).
The developer making the commit and the author are responsible for their changes
and should try to fix issues their commit causes.

[2 Coding Rules](https://ffmpeg.org#toc-Coding-Rules-1)

[2.1 Language](https://ffmpeg.org#toc-Language)
...
[2.1.1 SIMD/DSP](https://ffmpeg.org#toc-SIMD_002fDSP-1)
...
[2.1.2 Other languages](https://ffmpeg.org#toc-Other-languages)
...
written in the standard way described in the
[SIMD/DSP](https://ffmpeg.org#SIMD_002fDSP)section. This typically applies to code that needs to be inlined. - Objective-C where required for interacting with macOS-specific interfaces.

[2.2 Code formatting conventions](https://ffmpeg.org#toc-Code-formatting-conventions)
...
[2.2.1 Examples](https://ffmpeg.org#toc-Examples)
...
keywords:// Good if (condition) av_foo();

// Good for (size_t i = 0; i < len; i++) av_bar(i);
...
// Bad style if ( condition ) av_foo();

- No unnecessary parentheses, unless it helps readability:
// Good int fields = ilace ? 2 : 1;
...
// Good if (bits_pixel == 24) { avctx->pix_fmt = AV_PIX_FMT_BGR24; } else if (bits_pixel == 8) { avctx->pix_fmt = AV_PIX_FMT_GRAY8; } else return AVERROR_INVALIDDATA;

- Avoid assignments in conditions where it makes sense:
// Good video_enc->chroma_intra_matrix = av_mallocz(sizeof(*video_enc->chroma_intra_matrix) * 64) if (!video_enc->chroma_intra_matrix) return AVERROR(ENOMEM);

// Bad style if (!(video_enc->chroma_intra_matrix = av_mallocz(sizeof(*video_enc->chroma_intra_matrix) * 64))) return AVERROR(ENOMEM);

// Ok while ((entry = av_dict_iterate(options, entry))) av_log(ctx, AV_LOG_INFO, "Item '%s': '%s'\n", entry->key, entry->value);

- When declaring a pointer variable, the
`*`

goes with the variable not the type:// Good AVStream *stream;

// Bad style AVStream* stream;
...
[2.2.2 Vim configuration](https://ffmpeg.org#toc-Vim-configuration)
...
[2.2.3 Emacs configuration](https://ffmpeg.org#toc-Emacs-configuration)
...
[2.3 Comments](https://ffmpeg.org#toc-Comments)
...
[2.4 Naming conventions](https://ffmpeg.org#toc-Naming-conventions-1)
...
prefix must be used for variables and functions visible outside of file scope, but only used internally within a single library, e.g. ‘`ff_w64_demuxer`’. This prevents name collisions when FFmpeg is statically linked. - For variables and functions visible outside of file scope, used internally
across multiple libraries, use
`avpriv_`

as prefix, for example, ‘`avpriv_report_missing_feature`’. - All other internal identifiers, like private type or macro names, should be
namespaced only to avoid possible internal conflicts. E.g.
`H264_NAL_SPS`
...
for libavformat,`avcodec_`

for libavcodec,`swr_`

for libswresample, etc). Check the existing code and choose names accordingly. - Other public identifiers (struct, union, enum, macro, type names) must use their
library’s public prefix (
`AV`
...
[2.5 Miscellaneous conventions](https://ffmpeg.org#toc-Miscellaneous-conventions)
...
That means a bare value like ‘
`1.0`’ must mean 1 second, ‘`50m`’ means 50 milliseconds. For weight, gram shall be used.

[3 Development Policy](https://ffmpeg.org#toc-Development-Policy-1)

[3.1 Code behaviour](https://ffmpeg.org#toc-Code-behaviour)
...
[3.2 Patches/Committing](https://ffmpeg.org#toc-Patches_002fCommitting)
...
functional changes, since reviewers can use `git diff -w`

or
`git log -p --ignore-all-space`

to review only the functional parts of
...
[3.3 Code](https://ffmpeg.org#toc-Code)
...
[3.4 Library public interfaces](https://ffmpeg.org#toc-Library-public-interfaces)
...
extent described in [Major version bumps](https://ffmpeg.org#Major-version-bumps).
...
[3.4.1 Adding new interfaces](https://ffmpeg.org#toc-Adding-new-interfaces)
...
[Removing interfaces](https://ffmpeg.org#Removing-interfaces).
...
The naming of your new identifiers should follow the [Naming conventions](https://ffmpeg.org#Naming-conventions)
...
major version number, as described in [Major version bumps](https://ffmpeg.org#Major-version-bumps). Major
bumps are significant events that happen on a schedule - so if your change
strictly requires one you should add it under `#if`

preprocessor guards that
...
[3.4.2 Removing interfaces](https://ffmpeg.org#toc-Removing-interfaces-1)
...
removing the API - this is described in [Major version bumps](https://ffmpeg.org#Major-version-bumps).
...
[3.4.3 Major version bumps](https://ffmpeg.org#toc-Major-version-bumps-1)
...
[3.5 Documentation/Other](https://ffmpeg.org#toc-Documentation_002fOther)
...
[4 Submitting patches](https://ffmpeg.org#toc-Submitting-patches-1)

First, read the [Coding Rules](https://ffmpeg.org#Coding-Rules) above if you did not yet, in particular
...
Run the [Regression tests](https://ffmpeg.org#Regression-tests) before submitting a patch in order to verify
...
[5 New codecs or formats checklist](https://ffmpeg.org#toc-New-codecs-or-formats-checklist)
...
number) in
`libavcodec/version.h`or`libavformat/version.h`? - Did you register it in
`allcodecs.c`or`allformats.c`? - Did you add the AVCodecID to
`codec_id.h`? When adding new codec IDs, also add an entry to the codec descriptor list in`libavcodec/codec_desc.c`. - If it has a FourCC, did you add it to
`libavformat/riff.c`, even if it is only a decoder? - Did you add a rule to compile the appropriate files in the Makefile? Remember to do this even if you’re just adding a format to a file that is already being compiled by some other rule, like a raw demuxer.
- Did you add an entry to the table of supported formats or codecs in
`doc/general_contents.texi`? - Did you add an entry in the Changelog?
- If it depends on a parser or a library, did you add that dependency in configure?
- Did you
`git add`

the appropriate files before committing? - Did you make sure it compiles standalone, i.e. with
...
[6 Patch submission checklist](https://ffmpeg.org#toc-Patch-submission-checklist)

- Does
`make fate`

pass with the patch applied? - Was the patch generated with git format-patch or send-email?
- Did you sign-off your patch? (
`git commit -s`

) See[Sign your work](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/plain/Documentation/process/submitting-patches.rst)for the meaning of*sign-off*. - Did you provide a clear git commit log message?
...
[zzuf](http://caca.zoy.org/wiki/zzuf). Your decoder or demuxer should not crash, end in a (near) infinite loop, or allocate ridiculous amounts of memory when fed damaged data. - Did you test your decoder or demuxer against sample files?
Samples may be obtained at
[https://samples.ffmpeg.org](https://samples.ffmpeg.org). - Does the patch not mix functional and cosmetic changes?
...
[https://streams.videolan.org/upload/](https://streams.videolan.org/upload/). - Did you provide a verbose summary about what the patch does change?
...
- Consider adding a regression test for your code. All new modules should be covered by tests. That includes demuxers, muxers, decoders, encoders filters, bitstream filters, parsers. If its not possible to do that, add an explanation why to your patchset, its ok to not test if there’s a reason.
...
[7 Patch review process](https://ffmpeg.org#toc-Patch-review-process)
...
[8 Regression tests](https://ffmpeg.org#toc-Regression-tests-1)

Before submitting a patch (or committing to the repository), you should at least test that you did not break anything.

Running ’make fate’ accomplishes this, please see [fate.html](https://ffmpeg.org/fate.html) for details.
...
[8.1 Adding files to the fate-suite dataset](https://ffmpeg.org#toc-Adding-files-to-the-fate_002dsuite-dataset)
...
[8.2 Visualizing Test Coverage](https://ffmpeg.org#toc-Visualizing-Test-Coverage)
...
to generate coverage data in HTML format. - View
`lcov/index.html`
...
[8.3 Using Valgrind](https://ffmpeg.org#toc-Using-Valgrind)
...
[9 Maintenance process](https://ffmpeg.org#toc-Maintenance-process)

[9.1 MAINTAINERS](https://ffmpeg.org#toc-MAINTAINERS-1)
...
[9.2 Becoming a maintainer](https://ffmpeg.org#toc-Becoming-a-maintainer-1)
...
[10 Release process](https://ffmpeg.org#toc-Release-process-1)
...
**Point releases**are cut from**release**branches, which are named`release/X`

, with`X`

being the release version number.
...
[10.1 Criteria for Point Releases](https://ffmpeg.org#toc-Criteria-for-Point-Releases-1)

Changes that match the following criteria are valid candidates for inclusion into a point release:

- Fixes a security issue, preferably identified by a
**CVE number**issued by[http://cve.mitre.org/](http://cve.mitre.org/). - Fixes a documented bug in
[https://trac.ffmpeg.org](https://trac.ffmpeg.org). - Improves the included documentation.
- Retains both source code and binary compatibility with previous point releases of the same release branch.

The order for checking the rules is (1 OR 2 OR 3) AND 4.

[10.2 Release Checklist](https://ffmpeg.org#toc-Release-Checklist)

The release process involves the following steps:

- Ensure that the
`RELEASE`file contains the version number for the upcoming release. - Add the release at
[https://trac.ffmpeg.org/admin/ticket/versions](https://trac.ffmpeg.org/admin/ticket/versions). - Announce the intent to do a release to the mailing list.
- Make sure all relevant security fixes have been backported. See
[https://ffmpeg.org/security.html](https://ffmpeg.org/security.html). - Ensure that the FATE regression suite still passes in the release
branch on at least
**i386**and**amd64**(cf.[Regression tests](https://ffmpeg.org#Regression-tests)). - Prepare the release tarballs in
`bz2`

and`gz`

formats, and supplementing files that contain`gpg`

signatures - Publish the tarballs at
[https://ffmpeg.org/releases](https://ffmpeg.org/releases). Create and push an annotated tag in the form`nX`

, with`X`

containing the version number. - Propose and send a patch to the
**ffmpeg-devel**mailing list with a news entry for the website. - Publish the news entry.
- Send an announcement to the mailing list.

This document was generated on *April 16, 2026* using [ makeinfo](http://www.gnu.org/software/texinfo/).
