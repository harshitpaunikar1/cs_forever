Title: Logging Architecture | Kubernetes
Mapped Topic: Kubernetes concepts and tutorials
Source URL: https://kubernetes.io/docs/concepts/cluster-administration/logging/
Source Type: official_docs
Trust Score: 98
Fetched At: 2026-04-17T07:11:34+00:00
...
[cluster-level logging](https://kubernetes.io#cluster-level-logging-architectures).
...
This example uses a manifest for a `Pod`
...
(disabled by default)As an alpha feature, the kubelet can split out the logs from the two standard streams produced
...
With that feature gate enabled, Kubernetes 1.35 allows access to these
log streams directly via the Pod API. You can fetch a specific stream by specifying the stream name (either `Stdout`
...
query string. You must have access to read the `log`

subresource of that Pod.
...
(default 10Mi) and `containerLogMaxFiles`
...
after 10 MiB, running `kubectl logs`

returns at most 10MiB of data.
...
[pods](https://kubernetes.io/docs/concepts/workloads/pods/)) - The Kubernetes scheduler, controller manager, and API server run within pods
(usually
[static Pods](https://kubernetes.io/docs/tasks/configure-pod-container/static-pod/)). The etcd component runs in the control plane, and most commonly also as a static pod. If your cluster uses kube-proxy, you typically run this as a`DaemonSet`
...
by default. You use `journalctl`

to read the systemd journal; for example:
...
directory. If you want to have logs written elsewhere, you can indirectly
run the kubelet via a helper tool, `kube-log-runner`

, and use that tool to redirect
...
, read [System Logs](https://kubernetes.io/docs/concepts/cluster-administration/system-logs/#klog).
...
is the Kubernetes default location for these logs, several
cluster deployment tools set up Windows nodes to log to `C:\var\log\kubelet`
...
directory, bypassing the default logging mechanism (the components
...
It's important to note that the default location `/var/log/pods`
...
. Otherwise, if the logs are on a separate filesystem from `/var`
...
if you share the directory `/var/log`

into a static Pod for a component, node-level
...
can double how much storage you need on the node.
...
periodically.
However, it's more straightforward to use `stdout`
...
by the kubelet.Here are two example manifests that you can use to implement a sidecar container with a logging agent.
The first manifest contains a [ ConfigMap](https://kubernetes.io/docs/tasks/configure-pod-container/configure-pod-configmap/)
...
- Read about
[Kubernetes system logs](https://kubernetes.io/docs/concepts/cluster-administration/system-logs/) - Learn about
[Traces For Kubernetes System Components](https://kubernetes.io/docs/concepts/cluster-administration/system-traces/) - Learn how to
[customise the termination message](https://kubernetes.io/docs/tasks/debug/debug-application/determine-reason-pod-failure/#customizing-the-termination-message)that Kubernetes records when a Pod fails
