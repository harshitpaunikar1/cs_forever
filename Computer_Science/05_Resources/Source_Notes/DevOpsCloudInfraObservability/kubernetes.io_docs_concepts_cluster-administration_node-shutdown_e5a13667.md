Title: Node Shutdowns | Kubernetes
Mapped Topic: Kubernetes concepts and tutorials
Source URL: https://kubernetes.io/docs/concepts/cluster-administration/node-shutdown/
Source Type: official_docs
Trust Score: 98
Fetched At: 2026-04-17T07:11:41+00:00
Mapped From CSE.md Section: Part 2: G. DevOps / cloud / infra / observability

# Content

In a Kubernetes cluster, a [node](https://kubernetes.io/docs/concepts/architecture/nodes/)
...
package from Debian conflicts with node graceful shutdown in
its normal configuration.
If you use the default configuration of `unattended-upgrades`

, which customizes the server shutdown
grace period, then the kubelet fails to obtain the necessary lock to handle shutdown events properly.

This happens if the `shutdownGracePeriod`

value is greater than 30 seconds.
To avoid this, you can suppress part of the `unattended-upgrades`

configuration,
by making `/etc/systemd/logind.conf.d/unattended-upgrades-logind-maxdelay.conf`
...
(enabled by default)On Linux, the graceful node shutdown feature is controlled with the `GracefulNodeShutdown`
...
(enabled by default)On Windows, the graceful node shutdown feature is controlled with the `WindowsGracefulNodeShutdown`
...
the node will have to go through the [Non-Graceful Node Shutdown](https://kubernetes.io#non-graceful-node-shutdown) procedure mentioned above.
...
Once the kubelet is notified of a node shutdown, it sets a `NotReady`
...
[toleration](https://kubernetes.io/docs/concepts/scheduling-eviction/taint-and-toleration/) for
...
- Terminate
[critical pods](https://kubernetes.io/docs/tasks/administer-cluster/guaranteed-scheduling-critical-addon-pods/#marking-pod-as-critical)running on the node.
...
:Specifies the total duration that the node should delay the shutdown by. This is the total grace period for pod termination for both regular and

[critical pods](https://kubernetes.io/docs/tasks/administer-cluster/guaranteed-scheduling-critical-addon-pods/#marking-pod-as-critical).`shutdownGracePeriodCriticalPods`

:Specifies the duration used to terminate

[critical pods](https://kubernetes.io/docs/tasks/administer-cluster/guaranteed-scheduling-critical-addon-pods/#marking-pod-as-critical)during a node shutdown. This value should be less than`shutdownGracePeriod`
...
and will need to be re-scheduled.For example, if `shutdownGracePeriod=30s`
...
(enabled by default)To provide more flexibility during graceful node shutdown around the ordering
...
The [Graceful Node Shutdown](https://kubernetes.io#graceful-node-shutdown) feature, as described
...
(enabled by default)A node shutdown action may not be detected by kubelet's Node Shutdown Manager,
either because the command does not trigger the inhibitor locks mechanism used by
kubelet or because of a user error, i.e., the ShutdownGracePeriod and
ShutdownGracePeriodCriticalPods are not configured properly. Please refer to above
section [Graceful Node Shutdown](https://kubernetes.io#graceful-node-shutdown) for more details.

When a node is shutdown but not detected by kubelet's Node Shutdown Manager, the pods
that are part of a [StatefulSet](https://kubernetes.io/docs/concepts/workloads/controllers/statefulset/)
...
which states that `ControllerUnpublishVolume`
...
on the volume are called and succeed".
...
config field in `kube-controller-manager`

. Disabling the force detach on timeout feature means
...
via the [Non-Graceful Node Shutdown](https://kubernetes.io#non-graceful-node-shutdown) procedure mentioned above.

- Caution must be taken while using the
[Non-Graceful Node Shutdown](https://kubernetes.io#non-graceful-node-shutdown)procedure. - Deviation from the steps documented above can result in data corruption.

Learn more about the following:

- Blog:
[Non-Graceful Node Shutdown](https://kubernetes.io/blog/2023/08/16/kubernetes-1-28-non-graceful-node-shutdown-ga/). - Cluster Architecture:
