...
Source URL: https://hpbn.co/transport-layer-security-tls/
Source Type: open_book
Trust Score: 93
Fetched At: 2026-04-17T06:59:48+00:00
...
The SSL protocol was originally developed at Netscape to enable
...
([Figure 4-1](https://hpbn.co#ssl-layer)), enabling
...
[§](https://hpbn.co#encryption-authentication-and-integrity)Encryption, Authentication, and Integrity
...
detail in [TLS Handshake](https://hpbn.co#tls-handshake).
...
[Chain of Trust and
Certificate Authorities](https://hpbn.co#chain-of-trust-and-certificate-authorities). In addition, the server can also optionally
...
[§](https://hpbn.co#https-everywhere)HTTPS Everywhere
...
[§](https://hpbn.co#tls-handshake)TLS
Handshake

Before the client and the server can begin exchanging application data
...
these steps requires new packet roundtrips ([Figure 4-2](https://hpbn.co#tls-handshake-protocol)) between the client and the
server, which adds startup latency to all TLS connections.

[Figure 4-2](https://hpbn.co#tls-handshake-protocol) assumes the same
...
-
The server processes the key exchange parameters sent by the client, checks message integrity by verifying the MAC, and returns an encrypted

`Finished`

message back to the client. `168 ms`
...
messages are sent, but without waiting for the other side to do the same. This optimization reduces handshake overhead for new TLS connections to one roundtrip; see[Enable TLS False Start](https://hpbn.co#enable-tls-false-start). -
If the client has previously communicated with the server, an "abbreviated handshake" can be used, which requires one roundtrip and also allows the client and server to reduce the CPU overhead by reusing the previously negotiated parameters for the secure session; see

[TLS Session Resumption](https://hpbn.co#tls-session-resumption).
...
One of the design goals for [TLS
1.3](https://hpbn.co/tls13-spec) is to reduce the latency overhead for setting up the secure
connection: 1-RTT for new, and 0-RTT for resumed sessions!

[§](https://hpbn.co#rsa-diffie-hellman-and-forward-secrecy)RSA,
...
For the curious, the Wikipedia article on [Diffie-Hellman key exchange](https://hpbn.co/dhe-exchange) is a
...
[§](https://hpbn.co#application-layer-protocol-negotiation-alpn)Application Layer Protocol Negotiation (ALPN)
...
As a result, to enable easy deployment of custom protocols, we must
...
specification provides a special `Upgrade`

flow for this
very purpose. However, the use of `Upgrade`

can add an extra
network roundtrip of latency, and in practice is often unreliable in
the presence of many intermediaries; see [Proxies,
Intermediaries, TLS, and New Protocols on the Web](https://hpbn.co#proxies-intermediaries-tls-and-new-protocols-on-the-web).

For a hands-on example of HTTP Upgrade workflow, flip ahead to
[Upgrading to
HTTP/2](https://hpbn.co/http2/#upgrading-to-http2).
...
Application Layer Protocol Negotiation (ALPN), as the name implies,
is a TLS extension that addresses this need. It extends the TLS
handshake ([Figure 4-2](https://hpbn.co#tls-handshake-protocol)) and allows the peers to
...
field, containing the list of supported application protocols, into the`ClientHello`
...
field and returns a`ProtocolName`

field indicating the selected protocol as part of the`ServerHello`
...
[§](https://hpbn.co#server-name-indication-sni)Server Name
Indication (SNI)
...
To address the preceding problem, the Server Name Indication (SNI)
...
hostname sent in the `ClientHello`

message, select the
appropriate certificate, and complete the TLS handshake for the desired
host.

[§](https://hpbn.co#tls-session-resumption)TLS Session Resumption

The extra latency and computational costs of the full TLS handshake
...
The first Session Identifiers (RFC 5246) resumption mechanism was
introduced in SSL 2.0, which allowed the server to create and send a
32-byte session identifier as part of its Specifically, the client can include the session ID in the
Leveraging session identifiers allows us to remove a full roundtrip,
...
Session resumption is an important optimization both for HTTP/1.x
...
for both sides.
In fact, if the browser requires multiple connections to the same
...
performance.
None of the preceding problems are impossible to solve, and many
...
To address this concern for server-side deployment of TLS session
...
supports session tickets, the server can include a This session ticket is then stored by the client and can be included
in the The session identifiers and session ticket mechanisms are
respectively commonly referred to as In practice, deploying session tickets across a set of
...
[§](https://hpbn.co#session-identifiers)Session Identifiers`ServerHello`
...
message to indicate to the server that it
...
cache, then an abbreviated handshake ([Figure 4-3](https://hpbn.co#tls-resumed)) can take place. Otherwise, a full
new session negotiation is required, which will generate a new session
ID.
[§](https://hpbn.co#session-tickets)Session Tickets```
New Session
Ticket
```

record, which includes all of the negotiated session data
...
message of a subsequent session. Thus, all
...
*session caching* and
*stateless resumption* mechanisms. The main improvement of
...
[§](https://hpbn.co#chain-of-trust-and-certificate-authorities)Chain
of Trust and Certificate Authorities
...
Next, Alice receives a message from Charlie, whom she has never met,
...
private key and attached this signature with his message ([Figure 4-4](https://hpbn.co#cot)). In this case, Alice first checks
...
In practice, it would be impractical to store and manually verify each
...
authorities (CAs) to do this job for us ([Figure 4-5](https://hpbn.co#ca-cot)): the browser specifies which CAs to trust
...
Every browser allows you to inspect the chain of trust of your secure
connection ([Figure 4-6](https://hpbn.co#igvita-cot-chain)), usually accessible by clicking
...
[§](https://hpbn.co#certificate-revocation)Certificate Revocation

Occasionally the issuer of a certificate will need to revoke or
...
( Certificate Revocation List (CRL) is defined by RFC 5280 and
...
This process is simple and straightforward, but it has a number of
limitations:
...
The need to fetch the latest CRL list from the CA may block
certificate verification, which can add significant latency to the
TLS handshake.
The CRL fetch may fail due to variety of reasons, and in such
...
certificate chain.
As a result, the OCSP mechanism consumes less bandwidth and is able
...
The CA must ensure that the service is up and globally available
at all times.
Real-time OCSP requests may impair the client’s privacy because
the CA knows which sites the client is visiting.
The client must block on OCSP requests while validating the
certificate chain.
The browser behavior is, once again, undefined and typically
results in a "soft fail" if the OCSP fetch fails due to a network
timeout or other errors.
As a real-world data point, Firefox telemetry shows that OCSP
requests time out as much as 15% of the time, and add approximately
350 ms to the TLS handshake when successful—see For the reasons listed above, neither CRL or OSCP revocation
...
client:
Instead of the client making the OCSP request, it is the server
that periodically retrieves the signed and timestamped OCSP
response from the CA.
The server then appends (i.e. "staples") the signed OCSP
...
This role reversal is secure, because the stapled record is signed
...
The client may "hard-fail" revocation handling if the server
opts-in (by advertising the OSCP "Must-Staple" flag) and the
verification fails.
In short, to get both the best security and performance guarantees,
make sure to configure and test OCSP stapling on your servers.
[Figure 4-7](https://hpbn.co#revocation)) on how to
...
[§](https://hpbn.co#certificate-revocation-list-crl)Certificate
Revocation List (CRL)

[§](https://hpbn.co#online-certificate-status-protocol-ocsp)Online
Certificate Status Protocol (OCSP)

[hpbn.co/ocsp-performance](https://hpbn.co/ocsp-performance).
[§](https://hpbn.co#ocsp-stapling)OCSP Stapling

[§](https://hpbn.co#tls-record-protocol)TLS Record Protocol

Not unlike the IP or TCP layers below it, all data exchanged within a
TLS session is also framed using a well-defined protocol ([Figure 4-8](https://hpbn.co#tls-record-diagram)). The TLS Record
...
-
Received data is divided into blocks: maximum of 2

14bytes, or 16 KB per record. -
...
Picking the right record size for your application, if you have the
...
your application—skip ahead to [Optimize TLS Record Size](https://hpbn.co#optimize-tls-record-size) for full
details.

[§](https://hpbn.co#optimizing-for-tls)Optimizing for TLS

Deploying your application over TLS will require some additional work,
...
in.
Establishing and maintaining an encrypted channel introduces
...
handshake (explained As we noted earlier, public key cryptography is more computationally
...
Elliptic Curve Diffie-Hellman (ECDHE) is only a little more
...
To get the best results in your own deployments, make the best of
Speaking of optimizing CPU cycles, make sure to keep your servers
...
An unoptimized TLS deployment can easily add many additional
...
A well-tuned TLS deployment should add To get the best end-to-end performance, make sure to audit both
...
popular servers and CDNs, check out The best way to minimize both latency and computational overhead of
...
Verify that your server and proxy configurations are setup to allow
...
As we discussed in A simple way to accomplish this is to leverage the services of a
...
edge to origin.
Terminating the connection closer to the user is an optimization
...
repeat visitors.
Session identifiers, on which TLS session caching relies, were
...
Servers with multiple processes or workers should use a shared
session cache.
Size of the shared session cache should be tuned to your levels
of traffic.
A session timeout period should be provided.
In a multi-server setup, routing the same client IP, or the same
TLS session ID, to the same server is one way to provide good
session cache utilization.
Where "sticky" load balancing is not an option, a shared cache
...
tickets.
Check and monitor your TLS session cache statistics for best
performance.
In practice, and for best results, you should configure both session
...
Session resumption provides two important benefits: it eliminates an
...
To get the best of both worlds—a one roundtrip handshake for new and
...
allows the sender to send application data ( False Start does not modify the TLS handshake protocol, rather it
only affects the protocol timing of when the application data can be
sent. Intuitively, once the client has sent the
All application data delivered via TLS is transported within a
record protocol ( The smaller the record, the higher the framing overhead. However,
...
it can decrypt the data ( Small records incur overhead, large records incur latency, and there
...
connection:
When the connection is new and TCP congestion window is low, or
when the connection has been idle for some time (see When the connection congestion window is large and if we are
...
If the TCP connection has been idle, and even if Slow-Start
...
Next, the decision as to when the record size should be increased
...
Typically, configuring the TLS record size is not something we can
...
values.
Verifying the chain of trust requires that the browser traverse the
...
process.
How does the browser know from where to fetch the missing
...
Conversely, do not include unnecessary certificates, such as the
...
In practice, the size and depth of the certificate chain was a much
bigger concern and problem on older TCP stacks that initialized their
initial congestion window to 4 TCP segments—see That said, verify that your servers are using the latest TCP stack
...
optimization.
Every new TLS connection requires that the browser must verify the
...
To verify the status of the certificate the browser can use one of
several methods: OCSP responses can vary from 400 to 4,000 bytes in size.
...
Current OCSP Stapling implementations only allow a single OCSP
...
Most popular servers support OCSP stapling. Check the relevant
...
HTTP Strict Transport Security is an important security policy
mechanism that allows an origin to declare access rules to a compliant
browser via a simple HTTP header—e.g., " All requests to the origin should be sent over HTTPS. This
...
If a secure connection cannot be established, the user is not
allowed to circumvent the warning and request the HTTP version—i.e.
the origin is HTTPS-only.
HSTS converts the origin to an HTTPS-only destination and helps
...
they are dispatched!
Make sure to thoroughly test your TLS deployment before enabling
...
One of the shortcomings of the current system—as discussed in
The compromise of the DigiNotar certificate authority is one of
...
sites.
Public Key Pinning enables a site to send an HTTP header that
...
subsequent visits:
The origin can pin it’s leaf certificate. This is the most
...
browser.
The origin can pin one of the parent certificates in the
...
Picking the right strategy for which certificates to pin, which and
...
more information.
HPKP also exposes a "report only" mode that does not enforce the
...
To get the best security and performance guarantees it is critical
...
Mixed "active" content (e.g. scripts and stylesheets delivered
over HTTP) will be blocked by the browser and may break the
functionality of the site.
Mixed "passive" content (e.g. images, video, audio, etc.,
...
Audit your content and update your resources and links, including
third-party content, to use HTTPS. The CSP provides a highly configurable mechanism to control which
...
users.
As application developers we are shielded from most of the
...
Get best performance from TCP; see Upgrade TLS libraries to latest release, and (re)build servers
against them.
Enable and configure session caching and stateless resumption.
Monitor your session caching hit rates and adjust configuration
accordingly.
Configure forward secrecy ciphers to enable TLS False Start.
Terminate TLS sessions closer to the user to minimize roundtrip
latencies.
Use dynamic TLS record sizing to optimize latency and
throughput.
Audit and optimize the size of your certificate chain.
Configure OCSP stapling.
Configure HSTS and HPKP.
Configure CSP policies.
Enable HTTP/2; see [§](https://hpbn.co#reduce-computational-costs)Reduce Computational
Costs[TLS Handshake](https://hpbn.co#tls-handshake)). Then, once a shared secret is
established, it is used as a symmetric key to encrypt all TLS records.

[TLS Session
Resumption](https://hpbn.co#tls-session-resumption)—deploy, measure, and optimize its success rate.
...
[§](https://hpbn.co#enable-1-rtt-tls-handshakes)Enable 1-RTT TLS
Handshakes**at most one extra
roundtrip** for negotiating the TLS connection, regardless of
...
[§](https://hpbn.co#optimize-connection-reuse)Optimize Connection Reuse[§](https://hpbn.co#leverage-early-termination)Leverage Early
Termination[Primer on Latency and
Bandwidth](https://hpbn.co/primer-on-latency-and-bandwidth/), we may not be able to make our packets travel faster,
but we can make them travel a shorter distance. By placing our "edge"
servers closer to the user ([Figure 4-9](https://hpbn.co#early-termination)), we can significantly reduce
the roundtrip times and the total costs of the TCP and TLS handshakes.
[§](https://hpbn.co#configure-session-caching-and-stateless-resumption)Configure Session Caching and Stateless Resumption

[§](https://hpbn.co#enable-tls-false-start)Enable TLS False Start[Figure 4-10](https://hpbn.co#tls-false-start-diagram)) when the handshake is
only partially complete.
`ClientKeyExchange`

record, it already knows the encryption
...
[§](https://hpbn.co#optimize-tls-record-size)Optimize TLS Record Size[Figure 4-8](https://hpbn.co#tls-record-diagram)). The maximum size of each
...
overhead.
[Figure 4-11](https://hpbn.co#tls-split)). If any of those TCP packets get
...
[Slow-Start
Restart](https://hpbn.co/building-blocks-of-tcp/#slow-start-restart)), each TCP packet should carry exactly one TLS record,
and the TLS record should occupy the full maximum segment size
(MSS) allocated by TCP.
*time-to-first-{HTML byte, …, video
frame}*, and a larger record size optimizes throughput by
minimizing the overhead of TLS for long-lived streams.

`X`

KB of data have been transferred, and reset the record
size after `Y`

milliseconds of idle time.
[§](https://hpbn.co#optimize-the-certificate-chain)Optimize the
Certificate Chain[Slow-Start](https://hpbn.co/building-blocks-of-tcp/#slow-start). For newer
...
chains.
[§](https://hpbn.co#configure-ocsp-stapling)Configure OCSP Stapling[Certificate Revocation List
(CRL)](https://hpbn.co#certificate-revocation-list-crl), [Online Certificate Status
Protocol (OCSP)](https://hpbn.co#online-certificate-status-protocol-ocsp), or [OCSP
Stapling](https://hpbn.co#ocsp-stapling). Each method has its own limitations, but OCSP Stapling
...
[§](https://hpbn.co#enable-http-strict-transport-security-hsts)Enable HTTP Strict Transport Security (HSTS)*Strict-Transport-Security:
max-age=31536000*". Specifically, it instructs the user-agent to
enforce the following rules:

*max-age* specifies the lifetime of the specified HSTS
ruleset in seconds (e.g., `max-age=31536000`

is equal to a 365-day
lifetime for the advertised policy).
*includeSubdomains* indicates that the policy should
apply to all subdomains of the current origin.
[§](https://hpbn.co#enable-http-public-key-pinning-hpkp)Enable HTTP
Public Key Pinning (HPKP)[Chain of Trust and
Certificate Authorities](https://hpbn.co#chain-of-trust-and-certificate-authorities)—is our reliance on a large number of
...
[§](https://hpbn.co#update-site-content-to-https)Update Site Content
to HTTPS

[Content Security Policy](https://hpbn.co/csp) (CSP) mechanism can
...
makes no such assumptions. Try omitting the root
