...
Source URL: https://www.scs.stanford.edu/10au-cs144/notes/l12-print.pdf
Source Type: university_course_notes
Trust Score: 95
Fetched At: 2026-04-17T06:57:00+00:00
...
- For ﬂow bandwidths (x1, . . . , x n), fairness index :
f (x1, . . . , x n) = (∑n
i=1 xi)2
n ∑n
i=1 x2
...
• Fi: time when packet i ﬁnished transmission
• Fi = Si + Pi
...
• Thus: Fi = max( Fi−1, A i) + Pi
FQ Algorithm (cont)
• For multiple ﬂows
- Calculate Fi for each packet that arrives on each ﬂow
...
add/remove proxies, change value of NConsistent hashing [Karger]
...
- Let list of proxy addresses be p1, p 2, . . . p n
- For URL u, compute:
h1 ← HASH( p1, u ), h 2 ← HASH( p2, u ), . . .
- Sort h1, . . . h n. If hi is minimum, route request to pi.
